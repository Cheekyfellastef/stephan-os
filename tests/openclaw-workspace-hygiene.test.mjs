...
test('detects known OpenClaw repo-root workspace hard-block paths', () => {
  const projection = buildOpenClawWorkspaceHygieneProjection({
    hardBlockPaths: ['.openclaw/', 'COMMANDS.md', 'DREAMS.md', 'HEARTBEAT.md', 'IDENTITY.md', 'MEMORY.md', 'SOUL.md', 'TOOLS.md', 'USER.md', 'exec_output.txt', 'workspace_contents.txt', 'memory/'],
  ...
  assert.equal(projection.workspaceDirtCount, 12);
  assert.equal(projection.workspaceMutationAuthority, 'locked');
});


test('sanctioned external OpenClaw agent memory is not repo-root dirt', () => {
  const externalAgentMemory = '%USERPROFILE%\\.openclaw\\agents\\stephanos-scout-coder\\memory\\MEMORY.md';
  const projection = buildOpenClawWorkspaceHygieneProjection({
    paths: [externalAgentMemory],
    ignitionBlockedReason: 'Hard-block dirt detected',
  });

  assert.equal(isSanctionedOpenClawWorkspacePath(externalAgentMemory), true);
  assert.equal(isOpenClawWorkspaceDirtPath(externalAgentMemory), false);
...
test('migration command only targets known OpenClaw workspace paths and never arbitrary source files', () => {
  ...
  assert.match(OPENCLAW_WORKSPACE_CLEANUP_COMMAND, /Move-Item/);
  assert.match(OPENCLAW_WORKSPACE_CLEANUP_COMMAND, /Stephanos-openclaw-workspace/);
...
test('Builder Mesh blocks OpenClaw routing when workspace dirt blocks ignition', () => {
  ...
  assert.match(mesh.codexReason, /Codex remains fallback/i);
});
