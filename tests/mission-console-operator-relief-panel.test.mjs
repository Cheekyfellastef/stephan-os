...
test('Zero-Cost Builder Mesh is hosted inside existing Builder Harness panel, not a duplicate top-level pane', () => {
  ...
  assert.match(source, /OpenClaw Web Research Intake V1/);
  assert.match(source, /Copy VR Research Prompt/);
  assert.match(source, /Paste OpenClaw Web Research Result/);
  assert.match(source, /Run OpenClaw Web Intake Judgement/);
  assert.match(source, /Copy Cleaned Handoff Packet/);
  assert.match(source, /not trusted for canon until operator review/);
...
test('Mission Console registration callback identity is App bridge sourced and only marked invoked after call', () => {
  ...
  assert.match(source, /setRegistrationTraceState\(createRegistrationTraceState\(\{ callbackCallAttempted: 'yes'/);
  assert.match(source, /const callbackResult = onMissionConsoleInstanceRegistration\(\{ \.\.\.registrationPayload \}\);\n\s+const callbackReturnType = callbackResult === null \? 'null' : typeof callbackResult;\n\s+const callbackReceipt = callbackResult && typeof callbackResult === 'object' && callbackResult\.handled === true && callbackResult\.handler === 'app-bridge'/);
  assert.match(source, /setRegistrationTraceState\(createRegistrationTraceState\(\{[\s\S]*callbackInvoked: 'yes',[\s\S]*callbackReturnHandled: callbackReceipt \? 'yes' : 'no'/);
  ...
  assert.match(appSource, /const receivedPanelId = metadata\?\.panelId \|\| panelId/);
  assert.match(appSource, /const receivedSourceSurface = metadata\?\.sourceSurface \|\| sourceSurface/);
