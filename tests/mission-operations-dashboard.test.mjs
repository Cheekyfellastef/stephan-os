...
test('projection exposes complete mission, agent, Git, PR, approval, receipt, repair, deployment, timestamp, and next-action truth', () => {
  const projection = buildMissionOperationsProjection({
    ...
    repair: { currentRound: 1, maximumRounds: 3, history: [{ round: 1 }] },
    deployment: {
      sync: { status: 'success', completedAt: now.toISOString(), commitSha: 'c'.repeat(40) },
  ...
  assert.equal(projection.deployment.sync.commitSha, 'c'.repeat(40));
...
test('Mission Operations panel visibly renders every required operational truth family and bounded controls', async () => {
  ...
  for (const label of [
    ...
    assert.equal(controls.includes(label), true, `missing bounded Mission Operations control: ${label}`);
  }
...
test('server exposes bounded create, exact approval, cancellation, and read routes without generic event ingestion', async () => {
  ...
  assert.match(route, /router\.post\('\/missions', async/);
  assert.match(route, /router\.post\('\/missions\/:missionId\/approve', async/);
  assert.match(route, /router\.post\('\/missions\/:missionId\/cancel', async/);
  assert.match(route, /Cache-Control', 'no-store/);
  assert.doesNotMatch(route, /router\.post\('\/events'/);
  assert.doesNotMatch(route, /appendMissionEvent/);
});
