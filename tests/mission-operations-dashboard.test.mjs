...
test('projection exposes mission, agent, Git, PR, approval, evidence, timing, and next action truth', () => {
...
test('projection rejects unsafe PR links and makes blockers authoritative', () => {
  for (const prUrl of ['javascript:alert(1)', 'http://github.com/o/r/pull/1', 'https://example.com/fake-pr']) {
    assert.equal(buildMissionOperationsProjection({ missionId: 'safe-url', github: { prUrl } }, { now }).pullRequest.url, '');
  }
  const blocked = buildMissionOperationsProjection({ missionId: 'blocked', state: 'RUNNING', updatedAt: now.toISOString(), blockers: ['Head SHA changed.'] }, { now });
...
test('service returns explicit configuration state and adapts deterministic snapshots', async () => {
  ...
  assert.equal(feed.missions[0].mission.missionId, 'mission-service');
  assert.equal(feed.missions[0].receipts[0].sha256, 'c'.repeat(64));
});

test('Mission Operations panel renders all operational truth and bounded controls in the canonical panel', async () => {
  ...
  assert.match(source, /<MissionIntakeForm/);
  assert.match(source, /<MissionActionControls/);
  for (const label of ['Intended outcome:', 'Active agent', 'Supporting agents', 'Base branch', 'Head SHA', 'Worktree', 'Changed files', 'Pull request', 'PR state', 'Mergeable', 'Required checks', 'Approvals', 'Evidence receipts', 'Started', 'Updated', 'Elapsed', 'Next action:', 'Blockers:', 'Evidence warnings:']) assert.equal(source.includes(label), true, label);
  ...
  assert.match(controls, /type="password"/);
  assert.doesNotMatch(source, /requiredToken/);
...
test('server exposes bounded create, exact approval, cancellation, and read routes without generic event ingestion', async () => {
  ...
  assert.match(server, /app\.use\('\/api\/mission-operations', missionOperationsRouter\)/);
  assert.match(route, /router\.get\('\/'/);
  assert.match(route, /router\.post\('\/missions'/);
