...
test('large filtered ambient layers no longer repaint on every animation frame', () => {
  ...
  assert.match(performanceCss, /\.signal-orbit\s*\{[\s\S]*filter:\s*none\s*!important;[\s\S]*contain:\s*layout paint style;/);
...
test('performance layer cannot replace or restyle playback-bearing Music Tile nodes', () => {
  assert.doesNotMatch(performanceCss, /iframe|player-deck-card|listening-deck|listening-room|rating-button|data-rating/i);
});
