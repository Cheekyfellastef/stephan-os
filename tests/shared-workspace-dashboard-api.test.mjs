...
async function readyWorkspace() {
  const root = await tempDir('stephanos-backend-api-workspace-');
  for (const dir of ['goals', 'status', 'proof', 'capabilities', 'events']) await mkdir(join(root, dir), { recursive: true });
  await writeJson(root, 'status', 'status-1290.json', { ...createSharedWorkspaceStatusRecord({ statusId: 'status-1290', timestampUtc: NOW, status: 'CURRENT', summary: '#1290 Shared Workspace current', proofRefs: ['proof/status'] }), relatedGoal: '#1290' });
  await writeJson(root, 'proof', 'proof-1290.json', { ...createSharedWorkspaceProofRecord({ proofId: 'proof-1290', timestampUtc: NOW, status: 'PASS', summary: '#1290 proof current', refs: ['proof/shared-workspace'] }), relatedGoal: '#1290' });
  await writeJson(root, 'capabilities', 'openclaw.json', { ...createAgentCapabilityRecord({ agentId: 'openclaw', timestampUtc: NOW, proofRefs: ['proof/capability'] }), relatedGoal: '#1284 #1286' });
  return root;
}
...
test('backend dashboard feed adapter returns exact unavailable state without dumping env', async () => {
  ...
  const payload = await readBackendSharedWorkspaceDashboardFeed({ env: context.env, repoRoot: context.repoRoot });
  ...
  assert.equal(JSON.stringify(payload).includes(context.home), false);
});
...
test('backend dashboard feed redacts missing configured workspace root', async () => {
  ...
  const payload = await readBackendSharedWorkspaceDashboardFeed({ env: { ...context.env, STEPHANOS_SHARED_AGENT_WORKSPACE: missingRoot }, repoRoot: context.repoRoot });
  const serialized = JSON.stringify(payload);
  assert.equal(payload.state, 'unavailable');
  assert.equal(payload.reason, 'WORKSPACE_PATH_MISSING');
  assert.equal(payload.workspaceRoot, 'UNKNOWN');
...
test('backend dashboard feed route uses read-only adapter and maps unavailable to 503', async () => {
  const context = await isolatedContext();
  const layer = dashboardLayer({ env: context.env, repoRoot: context.repoRoot });
  let statusCode = 200;
  let payload = null;
...
test('backend dashboard feed adapter reads existing empty workspace without creating dashboard writes', async () => {
  ...
  const payload = await readBackendSharedWorkspaceDashboardFeed({ env: { ...context.env, STEPHANOS_SHARED_AGENT_WORKSPACE: root }, repoRoot: context.repoRoot, nowMs: Date.parse(NOW) });
  assert.equal(payload.route, '/api/shared-workspace/dashboard-feed');
...
test('backend dashboard feed adapter reads existing ready workspace records', async () => {
  ...
  const payload = await readBackendSharedWorkspaceDashboardFeed({ env: { ...context.env, STEPHANOS_SHARED_AGENT_WORKSPACE: root }, repoRoot: context.repoRoot, nowMs: Date.parse(NOW), staleAfterMs: 60_000 });
  assert.equal(payload.route, '/api/shared-workspace/dashboard-feed');
  assert.equal(payload.backendAdapter, 'shared-workspace-dashboard-feed-reader');
  assert.equal(payload.readOnly, true);
...
test('backend startup publisher loop only starts for existing configured workspace and remains stoppable', async () => {
  const context = await isolatedContext();
  const blocked = await startBattleBridgePublisherLoopForBackend({ env: context.env, repoRoot: context.repoRoot, runImmediately: false });
  ...
  const missing = await startBattleBridgePublisherLoopForBackend({ env: { ...context.env, STEPHANOS_SHARED_AGENT_WORKSPACE: missingRoot }, repoRoot: context.repoRoot, runImmediately: false });
  assert.equal(missing.started, false);
  assert.equal(missing.reason, 'WORKSPACE_PATH_MISSING');
  assert.equal(missing.workspaceRoot, 'UNKNOWN');

  const root = await emptyWorkspace();
  const started = await startBattleBridgePublisherLoopForBackend({ env: { ...context.env, STEPHANOS_SHARED_AGENT_WORKSPACE: root }, repoRoot: context.repoRoot, runImmediately: false, intervalMs: 30_000 });
