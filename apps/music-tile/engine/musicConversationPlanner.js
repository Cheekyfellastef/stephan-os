...
function signedTraitWeight(trait = null) {
  const weight = Math.max(0, Number(trait?.weight || 0));
...
function projectTeachingSet(tasteDNA, trait, beforeTeachings, afterTeachings, baselineTrait, now) {
  ...
  const expectedBeforeContributions = baselineContributions + beforeTeachings.length;
  const nonTeachingContributions = current
    ? Math.max(0, Math.max(0, Number(current.contributions || 0)) - expectedBeforeContributions)
    : 0;
  ...
  const record = {
    ...
    contributions: baselineContributions + afterTeachings.length + nonTeachingContributions,
