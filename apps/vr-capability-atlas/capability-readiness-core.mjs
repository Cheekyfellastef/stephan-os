...
export function scoreCapabilityEntry(entry = {}) {
  ...
  const evaluated = CAPABILITY_READINESS_STAGES.map((definition) => {
    const record = stages[definition.id] && typeof stages[definition.id] === 'object'
  ...
  return Object.freeze({
    ...
    complete: percent === TOTAL_WEIGHT && evaluated.every((stage) => stage.proven),
  });
}
...
export function capabilityReadinessForConcept(ledger, conceptId, { nowMs = Date.now() } = {}) {
  ...
  const entry = Array.isArray(ledger.capabilities)
    ? ledger.capabilities.find((candidate) => String(candidate?.id || '') === String(conceptId || ''))
    : null;
  if (!entry) {
    return Object.freeze({
      available: false,
...
export function capabilityReadinessSummary(result = {}) {
  if (!result.available) {
    return result.stale
      ? 'Capability evidence is stale, so Stephanos is refusing to guess.'
      : 'Capability evidence is unavailable, so Stephanos is refusing to guess.';
  }
  ...
  const provenText = proven.length ? `${proven.join(', ')} proven` : 'no proof stages proven';
  const pendingText = pending.length ? `${pending.join(', ')} still unproven` : 'all stages proven';
  return `${result.percent}% capability readiness · ${provenText} · ${pendingText}.`;
}
