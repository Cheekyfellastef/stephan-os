...
The review coordinator treats comments as evidence only when the comment actor matches the configured trusted coordinator login. Canonical-lane references must grammatically identify the PR being evaluated; mentions of replaced PRs do not disqualify the actual canonical subject, while an explicit assignment to another PR revokes the former lane. The latest trusted controller state is authoritative, with comment ID breaking GitHub's second-resolution timestamp ties, so a later queued, superseded, non-canonical or no-longer-active declaration revokes stale positive lane evidence. Every coordination pass requires an explicit positive PR number plus same-repository, open and `main`-targeting evidence, scans all open PRs to preserve single-canonical-lane enforcement before a requested PR may advance, refuses to act on a pagination-truncated estate, and accepts only safe positive decimal manual PR numbers.

Review receipts are causally ordered as well as head-bound. Every required workflow must have a successful exact-head completion timestamp, an external Codex receipt from the exact authenticated Codex GitHub App identity must be created strictly after the latest of those completions, and the durable coordinator receipt must follow that external receipt. Comment-only receipts must name the full 40-character reviewed SHA; unresolved prefixes are not exact-head evidence. Same-second issue comments are ordered by comment ID; a same-second review and issue comment are incomparable and fail closed until a later durable receipt exists. Plain-user aliases, lookalike actors and pre-proof or unauthenticated dispatch, receipt and escalation markers are ignored fail-closed.
