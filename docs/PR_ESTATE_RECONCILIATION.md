...
The review coordinator treats comments as evidence only when the comment actor matches the configured trusted coordinator login. Canonical-lane references must identify the PR being evaluated; a controller comment that names another PR as the active lane cannot make the current PR canonical. The latest trusted controller state is authoritative, so a later queued, superseded, non-canonical or no-longer-active declaration revokes stale positive lane evidence. Every coordination pass scans all open PRs to preserve single-canonical-lane enforcement before a requested PR may advance.
