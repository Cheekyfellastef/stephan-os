...
The reserve matching the next protected task may be consumed by that task while all unrelated reserves remain protected. An urgent repair may consume the emergency reserve, an exact-head review may consume the review reserve, and a Windows runtime proof may consume the runtime reserve.

## Trusted meter boundary

Codex dispatch requires an explicitly `AVAILABLE` meter observation with medium or high confidence, a valid observation timestamp, and a valid evaluation time. The default maximum observation age is 15 minutes. Missing, future-dated, stale, low-confidence, busy, degraded, unavailable, stalled, or unknown meter truth blocks dispatch.
...
- its one-hour safety deadline remains in the future;
