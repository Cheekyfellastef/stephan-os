...
The receipt includes the exact source head and tree, artifact-set digest, fixed runner identities, immutable proof references, teardown proof, zero-residual-authority proof and its own content digest. Its identifier is derived from the validated authorization ID. The aggregate proof estate must contain at least one correctly runner-prefixed content-addressed reference for each of the exact Linux and Windows runner identities; syntactically valid unrelated-runner evidence and a one-runner-only estate fail closed even when a caller recomputes the receipt digest.

`validateForgeShadowM3RunnerRuntimeReceipt()` materializes one inert receipt projection, revalidates its complete own-key estate, dense arrays and content digest, and returns only that deeply frozen projection. Every non-array receipt and expectation field must already be a primitive scalar; boxed values, zero-own-key wrappers and objects with mutable coercion hooks are rejected before digest or identity validation, so freezing the containing record cannot retain caller-owned `toString()` authority. Its supported expectation surface is limited to repository, source head, source tree and artifact-set digest because those identities exist in this canonical receipt. `expectedRuntimePlanDigest` is deliberately rejected rather than silently accepted: the construction receipt does not carry that field, so a validator cannot truthfully enforce it. Later Forge-aware routing therefore never receives the caller-owned object or rereads stateful accessors that could promote a mutated or widened receipt. Its exported inspection boundary is total: hostile proxies, getters, cyclic records and other caller-controlled inspection failures return `receipt-inspection-failed` rather than throwing into a router or adjudicator.
