...
- the decision receipt has the exact canonical field inventory, an allowed status consistent with its active/selected state, no candidates while `WAITING` or `APPROVAL_REQUIRED`, and empty contradiction codes;
- decision freshness;
