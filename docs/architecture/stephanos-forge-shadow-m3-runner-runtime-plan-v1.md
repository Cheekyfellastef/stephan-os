...
Both classes are fixed to Forge service `stephanos-forge-shadow` on `127.0.0.1:3340` and registration mode `one-time-local-contained`.
...
3. create isolated ephemeral runtime boundaries;
4. install fixed runner artifacts;
5. issue contained one-time local registration credentials;
6. register fixed runner identities;
7. run a bounded isolation canary;
8. publish immutable content-addressed proofs;
9. unregister runner identities;
10. destroy ephemeral workspaces and runtime boundaries;
11. prove zero residual credential or authority state.
