...
- the existing index entry must equal either the current `HEAD` blob or the target blob; an absent index entry is accepted only when the path is also absent from current `HEAD`;
...
- any worktree mismatch, distinct staged content, deletion, unreadable path, head movement, staging failure, or index mismatch blocks without a second merge attempt.
