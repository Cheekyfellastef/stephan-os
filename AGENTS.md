...
- **Hard terminal handoff:** a source-changing disposable builder must not report terminal success, relinquish its workspace, or allow its lease/workspace to disappear while remote publication is unproven unless the exact complete source artifact is already durable and independently readable. Terminal success requires either `REMOTE_BRANCH_VERIFIED` for the canonical branch or `PATCH_ESCROWED` / equivalent complete-file or exact-commit escrow bound to the exact parent, result tree, changed paths, hashes, tests and authority envelope.
- `BUILT_LOCAL` or `TESTED_LOCAL` without a durable artifact is non-terminal. Before returning a final completion message, the executor must export the complete verified diff/commit/tree through the existing escrow route while the workspace still exists. If no durable escrow write is available, it must remain blocked as `SOURCE_ARTIFACT_ESCROW_REQUIRED` and must not describe the local commit as safely completed or disposable.
- When the normal source publication route is unavailable but an operator-facing GitHub comment/result surface remains available, use that existing bounded surface to preserve a checksummed patch or complete-file bundle before the executor exits; never let the only copy of verified source bytes die with the workspace.
