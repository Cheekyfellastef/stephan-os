...
const USER_AGENT = 'stephanos-independent-merge-security-review-v1';
const MAX_PAGES = 20;
...
async function main() {
  ...
  if (!owner || !repo || !prNumber || !/^[a-f0-9]{40}$/.test(sourceHead) || !branch || baseBranch !== 'main' || !runId || !runAttempt) {
  ...
  const body = [
    PROTECTED_REVIEW_MARKER,
    '## Independent deterministic exact-head security review passed',
    '',
    '```json',
    JSON.stringify(receipt, null, 2),
    '```',
    '',
    'This clean receipt was produced before and independently of the operator approval environment. The reviewer executed trusted default-branch code, read the complete exact-head diff, required all exact-head workflows green and required zero unresolved review threads. It has no merge, mark-ready, source-write, Battle Bridge, OpenClaw or runtime authority.',
  ].join('\n');
