...
const deliveryModule = String.raw`import { readFile, readdir } from 'node:fs/promises';
...
const deliveryTests = String.raw`import assert from 'node:assert/strict';
...
write('shared/agents/sharedWorkspaceScopedDeliveryStatusV1.mjs', deliveryModule);
write('shared/agents/sharedWorkspaceScopedDeliveryStatusV1.test.mjs', deliveryTests);

const bridgePath = 'shared/agents/chatGptParticipantBridgeV1.mjs';
let bridge = read(bridgePath);
bridge = replaceOnce(
  bridge,
  `} from './sharedAgentWorkspaceStore.mjs';\n`,
  `} from './sharedAgentWorkspaceStore.mjs';\nimport { validateDeliveryStatusSubject } from './sharedWorkspaceScopedDeliveryStatusV1.mjs';\n`,
  'bridge delivery subject import',
);
bridge = replaceOnce(
  bridge,
  `export const CHATGPT_BRIDGE_READ_OPERATIONS = Object.freeze([\n  'READ_CURRENT_STATUS',\n  'READ_LATEST_PROOF',\n  'READ_OPERATOR_ATTENTION',\n]);`,
  `export const CHATGPT_BRIDGE_READ_OPERATIONS = Object.freeze([\n  'READ_CURRENT_STATUS',\n  'READ_LATEST_PROOF',\n  'READ_OPERATOR_ATTENTION',\n  'READ_DELIVERY_STATUS',\n]);`,
  'bridge read allowlist',
);
bridge = replaceOnce(
  bridge,
...
bridge = replaceOnce(
  ...
  'bridge delivery operation mapping',
);
bridge = replaceOnce(
  bridge,
  ...
  'bridge delivery subject validation',
);
...
bridgeTest = replaceOnce(
  ...
  'bridge test read allowlist',
);
bridgeTest += String.raw`
...
const relayPath = 'scripts/chatgpt-shared-workspace-github-relay.mjs';
...
relay = replaceOnce(
  ...
  'relay delivery projection imports',
);
relay = replaceOnce(
  relay,
  ...
  'relay delivery projection injection',
);
relay = replaceOnce(
  relay,
...
const operationsDoc = String.raw`# ChatGPT Shared Workspace live-status contract
...
`;
write('docs/operations/chatgpt-shared-workspace-live-status.md', operationsDoc);
...
for (const path of [
  ...
  if (content.includes('<<<<<<<') || content.includes('>>>>>>>')) throw new Error(`Conflict marker in ${path}`);
}

console.log('CHATGPT_SCOPED_LIVE_STATUS_PATCH_APPLIED=YES');
