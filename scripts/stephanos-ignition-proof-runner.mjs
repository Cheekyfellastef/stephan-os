...
function runOptionalEvidence(name, command, args) {
  ...
  return { name: step.name, command: step.command, status: step.status, error: step.error, value: step.stdout.trim() || step.stderr.trim(), passed: step.passed };
}
...
function buildRuntimeDomSignals(body) {
  ...
  const hasHtmlShell = /<html[^>]*>/i.test(body) && /<body[^>]*>/i.test(body);
  return { title, bodyContainsStephanos, hasHtmlShell, contentLength: body.length, bodySample: body.replace(/\s+/g, ' ').trim().slice(0, 240), passed: bodyContainsStephanos && hasHtmlShell };
}
...
function emptyRuntimeProbe(url, error = null) {
  return {
    ...
    domSignals: { title: null, bodyContainsStephanos: false, hasHtmlShell: false, contentLength: 0, bodySample: '', passed: false },
    error,
    passed: false,
  };
}
  ...
  return new Promise((resolveProbe) => {
    ...
    const request = get(url, { timeout: 5000 }, (response) => {
      ...
      response.on('end', () => {
        ...
        resolveProbe({ name: 'runtime-url-probe', url, startedAt, finishedAt: new Date().toISOString(), statusCode: response.statusCode, bodyContainsStephanos: domSignals.bodyContainsStephanos, domSignals, passed: httpPassed && domSignals.passed });
      });
    });
    request.on('timeout', () => { request.destroy(new Error('runtime probe timed out')); });
    request.on('error', (error) => { resolveProbe(emptyRuntimeProbe(url, error.message)); });
  });
}
...
function buildBlockedTranscript(error, runtimeUrl = process.env.STEPHANOS_IGNITION_RUNTIME_URL || DEFAULT_RUNTIME_URL) {
  ...
  return {
    ...
    proofScope: { cleanOrCurrentWorkspaceProof: false, safeGeneratedDirtProof: false, unsafeDirtBlockedProof: false, browserRuntimeProof: false, browserRuntimeDomProof: false, exactHeadProof: expectedHead ? exactHeadMatched : 'not-supplied', emergencyTranscriptProof: true },
    exactHead: { expected: expectedHead, actual: gitHead.value, matched: exactHeadMatched, branch: gitBranch.value },
    safetyBoundaries: { deletesSourceFiles: false, hidesBlockers: false, autoFixesUnknownDirt: false, merges: false, exactHeadApprovalRequiredBeforeMerge: true },
    operatorAction: 'Post this blocker transcript to #1281 / the new proof-helper PR. Keep the launcher window open if present, then inspect the concierge splash/support snapshot before retrying local proof.',
    evidence: { gitHead, gitBranch, gitStatusBefore },
    steps: [],
    runtime: emptyRuntimeProbe(runtimeUrl, 'proof runner failed before runtime probe completed'),
    error: error.message,
    transcriptPath,
  };
}
...
async function main() {
  ...
  const passed = steps.every((step) => step.passed) && runtime.passed && (!expectedHead || exactHeadMatched);
  const transcript = {
    marker: passed ? 'MILESTONE_5_IGNITION_BROWSER_RUNTIME_PROOF_PASSED' : 'MILESTONE_5_IGNITION_BROWSER_RUNTIME_PROOF_BLOCKED',
    proofScope,
    exactHead: { expected: expectedHead, actual: gitHead.value, matched: exactHeadMatched, branch: gitBranch.value },
    safetyBoundaries: { deletesSourceFiles: false, hidesBlockers: false, autoFixesUnknownDirt: false, merges: false, exactHeadApprovalRequiredBeforeMerge: true },
    operatorAction: passed ? 'Post this transcript to #1281 / the new proof-helper PR, then request exact-head approval before merge.' : 'Start Stephanos locally with npm run stephanos, then rerun this proof runner. If runtime or exact-head still fails, post the transcript as the blocker.',
    evidence: { gitHead, gitBranch, gitStatusBefore },
    steps,
    runtime,
    transcriptPath,
