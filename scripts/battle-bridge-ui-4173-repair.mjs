...
export async function runUi4173Repair({ sharedWorkspace = null, dryRun = true, spawnFn = spawn, stdout = process.stdout } = {}) {
  const facts = await collectLauncherReadinessLiveFacts({ sharedWorkspace });
  const readinessReport = planLauncherReadiness(facts);
  const result = evaluateUi4173Repair({ readinessReport, dryRun });
  if (result.allowedToStart && !dryRun) {
    const child = spawnFn(UI_4173_REPAIR_COMMAND_IDENTITY.command, UI_4173_REPAIR_COMMAND_IDENTITY.commandArgs, { cwd: process.cwd(), detached: true, stdio: 'ignore', shell: false });
    if (typeof child?.unref === 'function') child.unref();
    result.started = { pid: Number(child?.pid || 0) || null, commandId: UI_4173_REPAIR_COMMAND_IDENTITY.id };
  } else {
    result.started = null;
  }
