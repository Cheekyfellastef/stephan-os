...
function Read-MailboxGenerationHint {
    ...
    try {
        ...
        foreach ($candidate in @(
            ...
            if ($processSourceHead -notmatch '^[0-9a-f]{40}    param([Parameter(Mandatory = $true)][string[]]$Arguments)
...
[pscustomobject]@{
    ...
    observedAtUtc = (Get-Date).ToUniversalTime().ToString('o')
        ...
        if ($bestHead -notmatch '^[0-9a-f]{40}    param([Parameter(Mandatory = $true)][string[]]$Arguments)
    $text = (& $gitExe @Arguments 2>$null | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { Stop-Guardian -Blocker 'FIXED_GIT_READ_FAILED' }
    return $text
}
...
[pscustomobject]@{
 ...
 -or $bestObservedAt -eq [datetime]::MinValue) { return $unproven }
        return [pscustomobject]@{
            proven = $true
