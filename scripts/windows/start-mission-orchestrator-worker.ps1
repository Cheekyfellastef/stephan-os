...
$workerScript = Join-Path $repositoryRoot 'scripts\mission-orchestrator-finalizer-worker.mjs'
...
"[$([DateTime]::UtcNow.ToString('o'))] Mission Orchestrator finalizer worker starting from $repositoryRoot" | Out-File -LiteralPath $logPath -Append -Encoding utf8
...
"[$([DateTime]::UtcNow.ToString('o'))] Mission Orchestrator finalizer worker exited with code $exitCode" | Out-File -LiteralPath $logPath -Append -Encoding utf8
exit $exitCode
