...
function Get-VerifiedCleanupFallbackWorkerProcess {
    ...
    if (-not $cleanupTask) { Stop-WithBlocker 'MISSION_WORKER_CLEANUP_FALLBACK_PROCESS_NOT_PROVEN' }
...
function Stop-NewlyStartedOwnedWorker {
    ...
    if ($verifiedInvocationProcess) {
        ...
        if (-not $fallbackProcess) { Stop-WithBlocker 'MISSION_WORKER_CLEANUP_FALLBACK_PROCESS_NOT_PROVEN' }
    ...
    if ($ExpectedProcessId -gt 0) {
        ...
        if (-not $verifiedWorker) {
            Stop-WithBlocker $(if ($cleanupFallbackUsed) { 'MISSION_WORKER_CLEANUP_FALLBACK_PROCESS_NOT_PROVEN' } else { 'MISSION_WORKER_CLEANUP_PROCESS_IDENTITY_NOT_PROVEN' })
        }
    }
    ...
    if ($verifiedWorker) {
        ...
        if (-not $reverifiedWorker `
            ...
            Stop-WithBlocker $(if ($cleanupFallbackUsed) { 'MISSION_WORKER_CLEANUP_FALLBACK_PROCESS_IDENTITY_CHANGED' } else { 'MISSION_WORKER_CLEANUP_PROCESS_IDENTITY_CHANGED' })
        }
