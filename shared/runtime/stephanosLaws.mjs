...
export const stephanosLaws = Object.freeze([
  ...
  },
  {
    ...
    shortStatement: 'Draggable panes stay in one collision-aware plane; canon-mounted panes must not also be laid out by legacy in-page grid/absolute flows.',
    fullDescription: 'Stephanos panes participate in a shared layout plane across canon/system tile panes unless explicitly opted out. Tiles using canon movable panes must mount pane sections into the canon panel plane and remove those sections from normal document-flow layout ownership so there is one active pane plane. Restore/load, drag-end, resize, and reset layout flows must clamp panes to viewport bounds and resolve overlaps, including collapsed-header footprints, before persisting position truth.',
