...
const SEEDED_LANES = Object.freeze([
  ...
  createSeedLane({ id: 'agent-task-layer', title: 'Agent Task Layer', status: 'not-started', confidence: 0.58, weight: 10, why: 'No canonical cross-agent task model with durable lifecycle truth has been established.', evidence: ['stephanos-ui/src/state/missionDashboardModel.js'], blockers: ['Missing canonical task packet lifecycle.'], lastMilestone: 'Milestones track agent-layer themes but not canonical task model.' }),
  createSeedLane({ id: 'codex-handoff', title: 'Codex Handoff', status: 'partial', confidence: 0.63, weight: 9, why: 'Mission handoff export text exists, but structured codex task packet handoff is not yet first-class.', evidence: ['stephanos-ui/src/components/MissionDashboardPanel.jsx'], blockers: ['Depends on Agent Task Layer canonical packet model.'], lastMilestone: 'Clipboard handoff summaries available.' , dependsOn: ['agent-task-layer']}),
  createSeedLane({ id: 'openclaw-control', title: 'OpenClaw Control', status: 'blocked', confidence: 0.61, weight: 9, why: 'OpenClaw routing exists but policy-gated actuator harness + kill-switch readiness is incomplete.', evidence: ['stephanos-ui/src/state/missionConsoleTargetPolicy.js', 'stephanos-ui/src/components/openclaw/openclawIntegrationAdapter.js'], blockers: ['No explicit policy harness with kill-switch lifecycle evidence.'], lastMilestone: 'Mission console target policy present.', dependsOn: ['agent-task-layer'] }),
  createSeedLane({ id: 'verification-loop', title: 'Verification Loop', status: 'started', confidence: 0.79, weight: 10, why: 'Build and verify scripts exist, but closure loop to adjudicate agent task completion is incomplete.', evidence: ['scripts/build-stephanos-ui.mjs', 'scripts/verify-stephanos-dist.mjs'], blockers: ['Needs binding to agent task completion events.'], lastMilestone: 'Build + verify truth gates in use.', dependsOn: ['agent-task-layer'] }),
