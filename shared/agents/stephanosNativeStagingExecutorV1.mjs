...
function unique(values) { return Array.isArray(values) && values.length === new Set(values).size; }
...
export function validateStephanosNativeStagingRequest(request = {}) {
  const errors = [];
  if (!exactKeys(request, REQUEST_KEYS)) errors.push('request-shape-invalid');
  ...
  if (!text(request.branch) || request.branch === 'main') errors.push('branch-invalid');
  ...
  const allowed = (request.allowedFiles || []).map(canonicalPath);
  if (allowed.some((p) => !p)) errors.push('allowed-file-path-invalid');
  if (!plainArray(request.requiredTestIds) || !request.requiredTestIds.length || !unique(request.requiredTestIds) || request.requiredTestIds.some((id) => !SAFE_ID.test(text(id)))) errors.push('required-test-ids-invalid');
  if (!plainArray(request.sourceSnapshots) || request.sourceSnapshots.length !== allowed.length) errors.push('source-snapshots-invalid');
  const byPath = new Map();
  let totalBytes = 0;
  for (const snapshot of request.sourceSnapshots || []) {
    ...
    byPath.set(p, snapshot);
  }
  if (totalBytes > MAX_TOTAL_BYTES) errors.push('source-snapshot-total-too-large');
  for (const p of allowed) if (!byPath.has(p)) errors.push('source-snapshot-missing');
  return frozen({ valid: errors.length === 0, errors: frozen([...new Set(errors)]), allowedFiles: frozen(allowed) });
}
...
export function validateStephanosNativeModelResult(result = {}, request = {}) {
  ...
  const errors = requestValidation.valid ? [] : ['request-invalid'];
  if (!exactKeys(result, RESULT_KEYS)) errors.push('result-shape-invalid');
  ...
  if (!plainArray(result.replacements) || result.replacements.length < 1 || result.replacements.length > MAX_FILES) errors.push('replacements-invalid');
  ...
  for (const replacement of result.replacements || []) {
    ...
    const source = (request.sourceSnapshots || []).find((item) => canonicalPath(item.path) === p);
    ...
    if (content === null) errors.push('replacement-content-invalid');
      ...
      normalized.push(frozen({ path: p, beforeSha256: before, afterSha256: sha256(content), content }));
    }
  }
...
export function verifyStephanosNativeTestAndScopeProof(plan, proof = {}) {
  ...
  const errors = [];
  if (!exactKeys(proof, ['baseHead','leaseId','changedFiles','testReceipts','sourceAfter'])) errors.push('proof-shape-invalid');
  ...
  const changed = (proof.changedFiles || []).map(canonicalPath);
  ...
  for (const replacement of plan.replacements) {
    const record = after.find((item) => canonicalPath(item?.path) === replacement.path);
    if (!record || !exactKeys(record, ['path','sha256']) || text(record.sha256).toLowerCase() !== replacement.afterSha256) errors.push(`after-digest-mismatch:${replacement.path}`);
  }
  for (const untouched of plan.untouched) {
    const record = after.find((item) => canonicalPath(item?.path) === untouched.path);
    if (!record || !exactKeys(record, ['path','sha256']) || text(record.sha256).toLowerCase() !== untouched.sha256) errors.push(`untouched-drift:${untouched.path}`);
  }
