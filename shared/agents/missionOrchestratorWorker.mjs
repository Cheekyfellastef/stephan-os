...
function expiresAt(options = {}) {
  ...
  const lifetimeMs = Number.isInteger(options.authorizationLifetimeMs)
    ? options.authorizationLifetimeMs
    : 10 * 60 * 1000;
...
function receiptRequirement(operation) {
  const requirements = {
    'create-worktree': 'isolated worktree',
    commit: 'signed git commit',
    push: 'signed git push',
    'open-pr': 'pull request creation',
    'check-pr': 'pull request checks',
    'merge-pr': 'approved squash merge',
  };
  return requirements[operation] || `signed ${operation}`;
}

function operationRoot(state, operation) {
  return operation === 'create-worktree'
    ? text(state.repositoryRoot)
    : text(state.git?.worktreePath);
}

function operationClaims(state, operation, options = {}) {
  const id = actionId(state, operation);
  const issuedAt = nowIso(options);
  const claims = {
    authorizationId: id,
    missionId: state.missionId,
    operation,
    repository: state.repository,
    repositoryRoot: operationRoot(state, operation),
    defaultBranch: state.baseBranch || 'main',
    baseBranch: state.git?.baseBranch || state.baseBranch || 'main',
    branch: state.git?.branch || '',
    worktreePath: state.git?.worktreePath || '',
    allowedFiles: state.allowedFiles || [],
  ...
  if (['check-pr', 'merge-pr'].includes(operation)) claims.prNumber = state.pullRequest?.number;
  if (operation === 'merge-pr') claims.expectedHeadSha = state.pullRequest?.headSha || '';
  return claims;
}
...
function blocked(state, reason) {
  return {
    ...
    actionId: actionId(state, 'blocked'),
    missionId: state.missionId,
    actionKind: 'blocked',
    executable: false,
    blockers: [reason],
...
export function buildMissionWorkerAction(state, options = {}) {
  ...
  if (['COMPLETE', 'CANCELLED', 'BLOCKED', 'AWAITING_OPERATOR_APPROVAL'].includes(state.currentPhase)) {
    return {
      schemaVersion: 'stephanos.mission-worker-action.v1',
      actionId: actionId(state, 'wait'),
      missionId: state.missionId,
      actionKind: 'wait',
      executable: false,
      reason: state.currentPhase,
      finalVerdict: 'NO_ACTION_REQUIRED',
    };
  }

  if (SIGNED_OPERATION_PHASES.has(state.currentPhase)) {
    const operation = SIGNED_OPERATION_PHASES.get(state.currentPhase);
    if (!OPENCLAW_BRANCH_PATTERN.test(text(state.git?.branch))) {
      return blocked(state, 'Signed OpenClaw mutations require an openclaw/* mission branch.');
    }
    if (operation !== 'create-worktree' && !text(state.git?.worktreePath)) {
      return blocked(state, 'Signed GitHub operation requires an isolated worktree path.');
    }
    if (operation === 'merge-pr' && (!Number.isInteger(state.pullRequest?.number) || !SHA40_PATTERN.test(text(state.pullRequest?.headSha)))) {
      return blocked(state, 'Merge operation requires an exact pull request number and lowercase head SHA.');
    }
    const claims = operationClaims(state, operation, options);
    const preview = buildOpenClawGitHubOperation(claims);
    const allowedPreviewBlockers = operation === 'merge-pr'
      ? new Set([
    ...
    return {
      schemaVersion: 'stephanos.mission-worker-action.v1',
      actionId: claims.authorizationId,
      missionId: state.missionId,
      actionKind: 'signed-openclaw-operation',
      operation,
      owner: 'openclaw-standalone',
      activeWriter: 'none',
      receiptRequirement: receiptRequirement(operation),
      claims,
      approvalToken: operation === 'merge-pr' ? state.approval?.requiredToken || '' : '',
      executable: true,
      blockers: [],
      finalVerdict: 'READY_TO_ISSUE_AUTHORIZATION',
    };
  }
  ...
  if (['AGENT_IMPLEMENTATION', 'REPAIR_REQUIRED'].includes(state.currentPhase)) {
    return {
      ...
      worktreePath: state.git?.worktreePath || '',
      allowedFiles: state.allowedFiles || [],
      requiredTests: state.requiredTests || [],
      requiredEvidence: state.requiredEvidence || [],
      repairRound: state.repair?.currentRound || 0,
      executable: true,
      blockers: [],
      finalVerdict: 'READY_TO_DISPATCH_CODEX',
    };
  }
  ...
  if (state.currentPhase === 'LIVE_RUNTIME_INVESTIGATION') {
    return {
      schemaVersion: 'stephanos.mission-worker-action.v1',
      actionId: actionId(state, 'openclaw-readonly'),
      missionId: state.missionId,
      actionKind: 'agent-handoff',
      ...
      activeWriter: 'none',
      requiredEvidence: state.requiredEvidence || [],
      browserProofRequired: state.browserProofRequired === true,
      executable: true,
      blockers: [],
      finalVerdict: 'READY_TO_DISPATCH_OPENCLAW_READONLY',
    };
  }
  ...
  if (state.currentPhase === 'VERIFYING') {
    return {
      schemaVersion: 'stephanos.mission-worker-action.v1',
      actionId: actionId(state, 'verification'),
      missionId: state.missionId,
      actionKind: 'evidence-judgment',
      owner: 'verification-judge',
      activeWriter: 'none',
      requiredEvidence: state.requiredEvidence || [],
      receipts: state.evidenceReceipts || [],
      executable: true,
      blockers: [],
      finalVerdict: 'READY_TO_JUDGE_EVIDENCE',
    };
  }
  ...
  if (state.currentPhase === 'LOCAL_DEPLOYMENT') {
    return {
      schemaVersion: 'stephanos.mission-worker-action.v1',
      actionId: actionId(state, 'local-deployment'),
      missionId: state.missionId,
      actionKind: 'local-deployment',
      owner: 'openclaw-standalone',
      activeWriter: 'none',
      mergeCommitSha: state.pullRequest?.mergeCommitSha || '',
      steps: ['sync', 'build', 'verify', 'restart'].filter((step) => state.deployment?.[step]?.status !== 'success'),
      executable: true,
      blockers: [],
      finalVerdict: 'READY_FOR_LOCAL_DEPLOYMENT',
    };
  }
...
export function issueMissionWorkerAuthorization(action, privateKeyPem, options = {}) {
  if (action?.actionKind !== 'signed-openclaw-operation' || action.executable !== true) {
    return { finalVerdict: 'BLOCKED', blockers: ['Worker action is not an executable signed OpenClaw operation.'] };
  }
  ...
  return {
    schemaVersion: 'stephanos.mission-worker-request.v1',
    actionId: action.actionId,
    missionId: action.missionId,
    operation: action.operation,
    authorization,
    approvalToken: action.approvalToken || '',
    receiptRequirement: action.receiptRequirement,
    finalVerdict: 'MISSION_WORKER_REQUEST_ISSUED',
  };
}
