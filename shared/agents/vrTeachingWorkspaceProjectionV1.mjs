...
const EVIDENCE_PLANES = new Set([
  'NORMATIVE_OR_OFFICIAL_SPECIFICATION','OFFICIAL_AUTHORING_EVIDENCE','DIRECT_PUBLIC_SOURCE_EVIDENCE','PUBLIC_PRODUCT_OR_CREATOR_CLAIM','APPROVED_LOCAL_PACKAGE_EVIDENCE','OBSERVED_RUNTIME_OR_HEADSET_PROOF','STEPHANOS_INFERENCE_OR_PROPOSAL',
]);
function text(value) { return String(value ?? '').trim(); }
function list(value) { return Array.isArray(value) ? value.filter(Boolean) : []; }
function normalizedTextList(value) { return list(value).map(text).filter(Boolean); }
function hash(value) { return createHash('sha256').update(JSON.stringify(value)).digest('hex'); }
function replaceByIdentity(existing, fresh, identity) {
  const freshKeys = new Set(fresh.map(identity).filter(Boolean));
  const combined = [...existing.filter((item) => !freshKeys.has(identity(item))), ...fresh];
  const seen = new Set();
  return combined.filter((item) => { const key = identity(item); if (!key || seen.has(key)) return false; seen.add(key); return true; });
}
function normalizeTeaching(record = {}) {
  const teachingKey = text(record.teachingKey), candidateKey = text(record.candidateKey), sourceId = text(record.sourceId || record.canonicalSourceId), observedIdentity = text(record.observedIdentity || record.exactObservedIdentity);
  const evidencePlanes = normalizedTextList(record.evidencePlanes || [record.evidencePlane]);
  const proofRefs = normalizedTextList(record.proofRefs); const confidence = text(record.confidence); const errors = [];
  ...
  return { valid: !errors.length, errors, value: Object.freeze({ teachingKey,candidateKey,sourceId,observedIdentity,sourceCommentId:text(record.sourceCommentId),teachingCommentId:text(record.teachingCommentId),evidencePlanes:Object.freeze(evidencePlanes),confidence,licenceBoundary:text(record.licenceBoundary || record.licenseBoundary),reusableMethod:text(record.reusableMethod || record.capability),applicability:text(record.applicability),nonApplicability:text(record.nonApplicability),constraints:Object.freeze(normalizedTextList(record.constraints)),failureModes:Object.freeze(normalizedTextList(record.failureModes)),fallback:text(record.fallback),requiredProofLevel:text(record.requiredProofLevel),freshnessIdentity:text(record.freshnessIdentity || observedIdentity),parityProjectionState:text(record.parityProjectionState || 'NOT_APPLICABLE'),proofRefs:Object.freeze(proofRefs) }) };
}
export function projectVrTeachingIntoSharedWorkspace(input = {}) {
  const seen = new Set(), accepted = [], blocked = [], duplicates = [];
  for (const raw of list(input.teachingRecords)) { const normalized = normalizeTeaching(raw); if (!normalized.valid) { blocked.push({ teachingKey:text(raw?.teachingKey), errors:normalized.errors }); continue; } const item=normalized.value; const identity=`${item.teachingKey}|${item.candidateKey}|${item.sourceId}|${item.observedIdentity}`; if(seen.has(identity)){duplicates.push(item.teachingKey);continue;} seen.add(identity); accepted.push(item); }
  const newGraphCandidates=accepted.map((item)=>Object.freeze({teachingKey:item.teachingKey,candidateKey:item.candidateKey,sourceId:item.sourceId,observedIdentity:item.observedIdentity,evidencePlanes:item.evidencePlanes,confidence:item.confidence,reusableMethod:item.reusableMethod,requiredProofLevel:item.requiredProofLevel,proofRefs:item.proofRefs}));
  const stableIdentity=(item)=>text(item?.teachingKey)||text(item?.candidateKey);
  const capabilityGraphCandidates=replaceByIdentity(list(input.capabilityGraphCandidates),newGraphCandidates,stableIdentity);
  const methodLibrary=replaceByIdentity(list(input.methodLibrary),accepted.map((item)=>Object.freeze({...item})),(item)=>text(item?.teachingKey)||text(item?.candidateKey)||text(item?.reusableMethod));
  const proofRefs=[...new Set([...normalizedTextList(input.proofRefs),...accepted.flatMap((item)=>item.proofRefs)])];
  const projection=buildVrResearchWorkspaceProjection({...input,capabilityGraphCandidates,methodLibrary,proofRefs,updatedAt:text(input.updatedAt)||new Date().toISOString()});
