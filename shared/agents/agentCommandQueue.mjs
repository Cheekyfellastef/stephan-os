...
export function buildAgentCommandQueue({agentTaskProjection=null}={}){
  ...
  const items = [
    ...
    createItem({ itemId:`packet-${packetId}`, itemType:'openclaw_proposal_packet', label:'OpenClaw proposal packet', status: (surface.openClawProposalPacket?.packetStatus === 'ready_for_operator_review' || reviewQueue.queueStatus === 'ready_for_operator_review') ? 'ready_for_operator_review' : 'draft', sourceAgent:'openclaw', targetAgent:'operator', riskLevel:surface.openClawProposalRisk?.riskLevel || 'guarded', packetId, nextAction: reviewQueue.nextAction || surface.openClawReviewDecisionNextAction || 'Review proposal packet.' }),
    createItem({ itemId:`codex-prompt-${packetId}`, itemType:'codex_review_prompt', label:'Codex review prompt export', status: (codexExport.exportStatus === 'generated' || reviewDecision === 'ready_for_codex_review') ? 'ready_for_codex_review' : 'draft', sourceAgent:'openclaw', targetAgent:'codex', riskLevel:'guarded', packetId, nextAction: codexExport.nextAction || 'Copy prompt for Codex review.', evidence: asArray(codexExport.includedEvidence) }),
    ...
    createItem({ itemId:`evidence-${packetId}`, itemType:'evidence_request', label:'Evidence request', status: (evidenceRequest.requestStatus === 'requested' || asArray(evidenceRequest.missingEvidence).length > 0) ? 'needs_more_evidence' : 'archived', sourceAgent:'operator', targetAgent:'openclaw', riskLevel:'guarded', packetId, nextAction: evidenceRequest.nextAction || 'Attach evidence note.', blockers: asArray(evidenceRequest.missingEvidence) }),
  ...
  const readyCount = items.filter((i)=>['ready_for_review','ready_for_operator_review','ready_for_codex_review','ready_for_implementation_planning','ready_for_approval_review','dry_run_preview_ready'].includes(i.status)).length;
  return {
