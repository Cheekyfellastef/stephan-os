...
function reviewStarter(source, path, findings) {
  ...
  requirePattern(findings, source, /\$arguments\s*=\s*@\('stephanos-server\/backend-bootstrap\.mjs'\)[\s\S]*Start-Process\s+-FilePath\s+\$canonicalNode[\s\S]*-ArgumentList\s+\$arguments/, 'windows-backend-starter-process-not-pinned', 'Backend process launch must use canonical Node and the immutable bootstrap entry.', path);
