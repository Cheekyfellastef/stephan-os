...
const SCHEMA = 'stephanos.openclaw-provider-pool-pr1999-specialist-review.v1';
...
export function analyzeOpenClawProviderPoolPr1999SpecialistReviewV1(input = {}) {
  ...
  if (!exactLineage(input.lineageEvidence, sourceHead, baseSha)) return Object.freeze({
    schemaVersion: SCHEMA, eligible: true, clean: false,
  ...
  for (const path of OPENCLAW_PROVIDER_POOL_PR1999_SPECIALIST_PATHS_V1) {
    ...
    if (path.endsWith('.test.mjs')) reviewProviderPoolTests(candidates[0].content, path, findings);
    else reviewProviderPool(candidates[0].content, path, findings);
    proofRefs.push(`proofs/openclaw-provider-pool-pr1999-specialist/${path}@${sourceHead}#${candidates[0].blobSha}`);
  }

  const clean = findings.length === 0;
  return Object.freeze({
    schemaVersion: SCHEMA, eligible: true, clean,
    reviewedPaths: OPENCLAW_PROVIDER_POOL_PR1999_SPECIALIST_PATHS_V1,
