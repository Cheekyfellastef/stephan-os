...
const base = {
  missionId: 'worker-test-mission',
  revision: 4,
  title: 'Worker test mission',
  intendedOutcome: 'A bounded change is promoted safely.',
  ...
  git: {
    branch: 'openclaw/worker-test-mission',
    baseBranch: 'main',
    worktreePath: 'C:\\Users\\Operator\\Documents\\GitHub\\stephan-os-worktrees\\worker-test-mission',
...
test('creates a bounded signed worktree action on the OpenClaw branch family', () => {
  ...
  assert.equal(action.claims.branch, base.git.branch);
...
test('blocks signed mutation when mission branch is outside openclaw/*', () => {
  const action = buildMissionWorkerAction({
    ...base,
    git: { ...base.git, branch: 'orchestrator/worker-test-mission' },
  }, { now });
...
test('issues an Ed25519 single-use authorization that the existing executor verifies', () => {
  ...
  const verification = verifyOpenClawGitHubAuthorization(
  ...
  assert.equal(verification.claims.singleUse, true);
});
...
test('local deployment resumes only unfinished ordered steps', () => {
  const action = buildMissionWorkerAction({
    ...base,
    currentPhase: 'LOCAL_DEPLOYMENT',
    pullRequest: { ...base.pullRequest, mergeCommitSha: 'b'.repeat(40) },
    deployment: { ...base.deployment, sync: { status: 'success' } },
  }, { now });
