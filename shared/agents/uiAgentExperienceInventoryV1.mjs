...
export const UI_AGENT_SURFACE_CLASSES = Object.freeze(['REGISTERED_APP','EMBEDDED_STEPHANOS_SURFACE','DEVICE_PRESENTATION','TEXT_OR_MESSAGING_PRESENTATION','VOICE_PRESENTATION','SPATIAL_PRESENTATION','FUTURE_PRODUCT_SURFACE']);
export const UI_AGENT_EXPERIENCE_DEBT_CLASSES = Object.freeze(['VISUAL_DRIFT','INCONSISTENT_COMPONENT','POOR_INFORMATION_HIERARCHY','CONTROL_CLUTTER','TOUCH_FRICTION','RESPONSIVE_DEFECT','ACCESSIBILITY_DEFECT','MOTION_DEFECT','STATE_TRUTH_DEFECT','EMPTY_OR_ERROR_STATE_DEFECT','CROSS_SURFACE_INCONSISTENCY','SPATIAL_READINESS_GAP','PERFORMANCE_PERCEPTION_GAP','UNKNOWN']);
export const UI_AGENT_REQUIRED_PROOF_CLASSES = Object.freeze(['SOURCE_INVENTORY','DESKTOP_VISUAL','TABLET_VISUAL','PHONE_VISUAL','KEYBOARD_INTERACTION','TOUCH_INTERACTION','REDUCED_MOTION','LOADING_ERROR_OFFLINE','WINDOWS_EDGE_INTERACTION','SPATIAL_OR_QUEST']);
...
const CANONICAL_PRODUCT_SURFACE_IDS = Object.freeze(['stephanos-landing-page','ai-console','goal-dashboard','music-tile','vr-research-lab','vr-link','sovereignty','wealth','privacy','trading-laboratory','autonomous-build-controls','command-deck','ignition-splash','desktop-browser','windows-edge','ipad','iphone','whatsapp','voice','quest3-spatial']);
...
function text(value, fallback = '') { if (value === null || value === undefined) return fallback; const output = String(value).trim(); return output || fallback; }
...
function sourceRef(value) { const candidate = text(value); return Boolean(candidate && !ABSOLUTE_SOURCE_REF.test(candidate) && !candidate.split(/[\\/]/).includes('..') && SAFE_SOURCE_REF.test(candidate)); }
...
function surface(input = {}) {
  ...
  return Object.freeze({
    ...
    experienceVersion:text(record.experienceVersion,'UNASSESSED'), componentVersion:text(record.componentVersion,'UNASSESSED'), responsiveCoverage:text(record.responsiveCoverage,'UNKNOWN'), accessibilityCoverage:text(record.accessibilityCoverage,'UNKNOWN'), motionCoverage:text(record.motionCoverage,'UNKNOWN'), loadingEmptyErrorCoverage:text(record.loadingEmptyErrorCoverage,'UNKNOWN'), inputMethods:freezeList(list(record.inputMethods)), lastVisualProof:text(record.lastVisualProof), lastInteractionProof:text(record.lastInteractionProof), knownExperienceDebt:freezeList(normalizedExperienceDebt(record.knownExperienceDebt)), severity:text(record.severity,'UNKNOWN'), recommendedNextImprovement:text(record.recommendedNextImprovement,'AUDIT_REQUIRED'),
  });
}
function inferredSurfaceFromRegisteredApp(appId) { const normalized = text(appId); const surfaceId = ({ stephanos:'stephanos-landing-page','goal-dashboard':'goal-dashboard','music-tile':'music-tile','vr-research-lab':'vr-research-lab',wealthapp:'wealth',cockpit:'command-deck' })[normalized] || `app:${normalized}`; return surface({ surfaceId, surfaceClass:'REGISTERED_APP', ownerGoal:'UNKNOWN', registrationRef:'apps/index.json', inputMethods:['POINTER','KEYBOARD'], knownExperienceDebt:['UNKNOWN'] }); }
export function validateUiAgentSharedPrimitive(record) { const errors=[]; if (!record || typeof record !== 'object' || Array.isArray(record)) return { valid:false, errors:['record-must-be-object'] }; if (record.schemaVersion !== UI_AGENT_SHARED_PRIMITIVE_SCHEMA_VERSION) errors.push('schema-version-mismatch'); if (!safeId(record.primitiveId)) errors.push('primitiveId-invalid'); if (!sourceRef(record.sourceRef)) errors.push('sourceRef-invalid'); if (!text(record.selectorOrExport)) errors.push('selectorOrExport-required'); if (!text(record.role)) errors.push('role-required'); return Object.freeze({ valid:errors.length===0, errors:Object.freeze(errors) }); }
...
export function buildUiAgentExperienceInventory(input = {}) {
  ...
  const observedAtUtc=text(input.observedAtUtc); const observedAtMs=Date.parse(observedAtUtc); const nowMs=Number.isFinite(input.validationOptions?.nowMs)?input.validationOptions.nowMs:Number.isFinite(input.nowMs)?input.nowMs:Date.now(); if (!timestamp(observedAtUtc)) validationErrors.push('observedAtUtc-invalid'); else if (observedAtMs>nowMs) validationErrors.push('observedAtUtc-future-dated');
  const inventoryId=`ui-inventory-${hash({ registeredApps,surfaces,sharedPrimitives,observedAtUtc }).slice(0,24)}`;
  return Object.freeze({ schemaVersion:UI_AGENT_EXPERIENCE_INVENTORY_SCHEMA_VERSION, inventoryId, participantId:'user-interface-agent', lifecycleState:'READ_ONLY_CANDIDATE', observedAtUtc, registeredApps, surfaces, sharedPrimitives, coverage:Object.freeze({ canonicalTargetCount:CANONICAL_PRODUCT_SURFACE_IDS.length, coveredCanonicalCount:coveredCanonical.length, coveredCanonical:freezeList(coveredCanonical), missingCanonical:freezeList(missingCanonical) }), nextMilestone:validationErrors.length===0 && missingCanonical.length===0?'M3_PUBLISH_CANONICAL_EXPERIENCE_CONTRACT_AND_DESIGN_MAP':'M2_COMPLETE_SOURCE_AND_PRESENTATION_SURFACE_DISCOVERY', authority:Object.freeze({ sourceMutationAllowed:false, implementationAllowed:false, mergeAllowed:false, deploymentAllowed:false, productAuthority:false }), valid:validationErrors.length===0, validationErrors:freezeList(validationErrors) });
}
