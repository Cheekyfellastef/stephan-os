...
export const OPENCLAW_WORKSPACE_MIGRATION_COMMAND = `$workspace = Join-Path $env:USERPROFILE 'Documents\\Stephanos-openclaw-workspace'; $migration = Join-Path $workspace ('root-migration-' + (Get-Date -Format 'yyyyMMdd-HHmmss')); New-Item -ItemType Directory -Force -Path $migration | Out-Null; $paths = @(${KNOWN_OPENCLAW_WORKSPACE_DIRT_PATHS.map((path) => `'${path}'`).join(',')}) | Where-Object { Test-Path -LiteralPath $_ }; foreach ($path in $paths) { Move-Item -LiteralPath $path -Destination $migration }; Write-Host "Moved OpenClaw workspace files to $migration without deleting memory."`;
