...
const anchor = '44db9156eeee26f1ab146fb447c0c59851ca3dcb';
...
test('requires one canonical cancellation deadline representation across writer and reader', () => {
  ...
  assert.equal(blob(formerRestart), '284e05ad75ea347911b86a837d16d0fa024b5012');
  assert.equal(Buffer.byteLength(formerRestart), 47111);
...
test('guards StartTime and launch-receipt failures with the exact in-memory child capability', () => {
  ...
  assert.match(launcher, /\$workerProcessStarted = \$true[\s\S]*\$ownedWorkerProcess = \$workerProcess[\s\S]*\$workerProcess\.StartTime\.ToUniversalTime\(\)[\s\S]*Write-BoundedAtomicJson -Path \$receiptPath/);
  for (const mutation of [
    launcher.replace('[object]::ReferenceEquals($Process, $OwnedProcess)', '$true'),
    launcher.replace('$workerProcessStarted = $true', '$workerProcessStarted = $false'),
    launcher.replace('$ownedWorkerProcess = $workerProcess', '$ownedWorkerProcess = $null'),
    launcher.replace('Stop-ExactOwnedWorkerProcess `', '# Stop-ExactOwnedWorkerProcess `'),
    launcher.replace('$Process.Kill()', '# $Process.Kill()'),
    launcher.replace('throw $launchFailure', 'return'),
  ]) {
...
test('top-level specialist pins and routes the watchdog reviewer before the legacy core', async () => {
  ...
  assert.match(source, /WORKER_WATCHDOG_BLOB_SHA = '47f2b8135689f03dfb97e73da58b64e9be82a016'/);
