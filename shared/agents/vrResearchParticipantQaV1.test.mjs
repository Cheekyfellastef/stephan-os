...
const VERIFIED_PROOFS = new Set(['evidence/receipts/vr-research-test','evidence/receipts/vr-gap-observation-proof']);
...
function projection(overrides = {}) {
  return buildVrResearchWorkspaceProjection({
    sourceRegistry:{ schema_version:'test-1', sources:[
    ...
    workspaceModel:{ schemaVersion:'lab-test-v1', targets:[{ name:'Starfield VR' }], experiments:[{ id:'experiment-openxr-readiness', title:'OpenXR readiness proof', status:'queued', hypothesis:'A bounded readiness probe can separate provider truth from flat fallback.', relatedTechniques:['openxr'] }] },
    updatedAt,
    ...
    blockers:[{ id:'spatial-runtime-proof', summary:'Quest 3 runtime proof is still required before Spatial Bridge promotion.', owner:'battle-bridge-vr-worker', evidencePlane:'OBSERVED_RUNTIME_OR_HEADSET_PROOF' }],
    runtimeEvidenceRequests:[{ id:'quest3-airlink-proof', summary:'Collect exact Quest 3 Air Link runtime evidence.' }],
    discoveryCandidates:[{ id:'candidate-new-openxr-technique', summary:'Needs provenance and licence triage.' }],
    proofRefs:['evidence/receipts/vr-research-test'],
    ...overrides,
  });
}
function request(questionClass, index, overrides = {}) {
  return createVrResearchQuestion({ questionId:`vr-question-${String(index + 1).padStart(2, '0')}`, askerParticipantId:'chatgpt-bridge', questionClass, questionText:`Answer the bounded VR research question for ${questionClass}.`, createdAtUtc:updatedAt, ...overrides });
}
...
test('all ten proving classes ground only when canonical projection proofs are verified', () => {
  const p = projection();
  const requests = [request('SOURCE_STACK',0),request('NEXT_EXPERIMENT',1),request('EVIDENCE_PLANE',2,{ subjectRef:'mutar' }),request('AUTHORING_VS_RUNTIME',3),request('VORPX_BASELINE',4),request('SKYRIM_PARITY',5),request('LICENCE_BOUNDARIES',6),request('SPATIAL_BRIDGE_BLOCKERS',7),request('NEXT_BOUNDED_GOAL',8),request('KNOWN_UNKNOWNS',9)];
...
test('proof references alone never ground an answer without trusted proof authority', () => {
  const noVerifier = answerVrResearchQuestion(request('SOURCE_STACK',0), projection(), { nowMs, answeredAtUtc });
...
test('ref-only boolean proof cannot ground projection facts', () => {
  const result = answerVrResearchQuestion(request('SOURCE_STACK',0), projection(), qaInput({ proofVerifier:(ref) => VERIFIED_PROOFS.has(ref) }));
  assert.equal(result.answer.answerVerdict, 'GAP_KNOWLEDGE');
  assert.deepEqual(result.answer.evidenceRefs, []);
});

test('proof verifier exceptions fail closed rather than grounding an answer', () => {
  const result = answerVrResearchQuestion(request('SOURCE_STACK',0), projection(), qaInput({ proofVerifier() { throw new Error('proof backend unavailable'); } }));
  assert.equal(result.answer.answerVerdict, 'GAP_KNOWLEDGE');
  assert.deepEqual(result.answer.evidenceRefs, []);
});
...
test('a projection-bound proof ref cannot be replayed after projection content changes', () => {
  ...
  const first = answerVrResearchQuestion(request('SOURCE_STACK',0), original, qaInput({ proofVerifier:pinnedVerifier }));
  assert.equal(first.answer.answerVerdict, 'ANSWERED_GROUNDED');
  const tampered = { ...original, facts:[...original.facts, { subjectRef:'tampered', evidencePlane:'DIRECT_PUBLIC_SOURCE_EVIDENCE', claim:'changed after proof' }] };
...
test('direct answer facts are sanitized before returning to Q&A consumers', () => {
  const p = projection({ facts:[{ subjectRef:'mutar', evidencePlane:'DIRECT_PUBLIC_SOURCE_EVIDENCE', claim:'Public implementation evidence exists.', apiKey:'super-secret-value', summary:{ token:'nested-secret' } }] });
  const result = answerVrResearchQuestion(request('EVIDENCE_PLANE',2,{ subjectRef:'mutar' }), p, qaInput());
  assert.equal(result.answer.answerVerdict, 'ANSWERED_GROUNDED');
  assert.deepEqual(result.answer.facts, [{ subjectRef:'mutar', evidencePlane:'DIRECT_PUBLIC_SOURCE_EVIDENCE', claim:'Public implementation evidence exists.' }]);
...
test('verified grounded answers publish through the existing Shared Workspace contract with zero authority', () => {
  ...
  const workspace = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { correlationId:'vr-round-001', proofVerifier, validationOptions:{ nowMs } });
...
test('Workspace publication also requires proof verification rather than path-shaped refs', () => {
  const q = request('SOURCE_STACK',0); const answered = answerVrResearchQuestion(q, projection(), qaInput());
  const withoutVerifier = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { validationOptions:{ nowMs } });
  assert.equal(withoutVerifier.validation.valid, false); assert.ok(withoutVerifier.validation.errors.includes('missing-proofRefs'));
  const rejectingVerifier = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { proofVerifier:() => false, validationOptions:{ nowMs } });
  assert.equal(rejectingVerifier.validation.valid, false); assert.ok(rejectingVerifier.validation.errors.includes('missing-proofRefs'));
});

test('evidence-free gap messages require a separately verified proof before publication', () => {
  const q = request('EVIDENCE_PLANE',2,{ subjectRef:'unproven-provider' }); const answered = answerVrResearchQuestion(q, projection(), qaInput());
  assert.equal(answered.answer.answerVerdict, 'GAP_KNOWLEDGE'); assert.deepEqual(answered.answer.evidenceRefs, []);
  const unproven = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { validationOptions:{ nowMs } });
  assert.equal(unproven.validation.valid, false); assert.ok(unproven.validation.errors.includes('missing-proofRefs'));
  const proven = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { proofRefs:['evidence/receipts/vr-gap-observation-proof'], proofVerifier, validationOptions:{ nowMs } });
...
test('directly deserialized request extras are never serialized into Shared Workspace body', () => {
  const q = { ...request('SOURCE_STACK',0), apiKey:'super-secret-value', arbitraryMutation:true }; const answered = answerVrResearchQuestion(q, projection(), qaInput());
  const workspace = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { proofVerifier, validationOptions:{ nowMs } });
  assert.equal(workspace.validation.valid, true, workspace.validation.errors.join(', '));
  const body = JSON.parse(workspace.record.body);
  assert.equal(Object.hasOwn(body.request,'apiKey'), false); assert.equal(Object.hasOwn(body.request,'arbitraryMutation'), false); assert.equal(workspace.record.body.includes('super-secret-value'), false);
});
...
test('caller-added answer fields are also excluded from Workspace serialization', () => {
  ...
  const workspace = createVrResearchQaWorkspaceAnswerRecord(q, forged, { proofVerifier, validationOptions:{ nowMs } });
  assert.equal(workspace.validation.valid, true, workspace.validation.errors.join(', '));
  const body = JSON.parse(workspace.record.body);
  assert.equal(Object.hasOwn(body.answer,'apiKey'), false); assert.equal(Object.hasOwn(body.answer,'mergeAllowed'), false); assert.equal(workspace.record.body.includes('answer-secret'), false);
});

test('nested fact fields are sanitized before Workspace serialization', () => {
  const q = request('SOURCE_STACK',0); const answered = answerVrResearchQuestion(q, projection(), qaInput());
  const forged = { ...answered.answer, facts:[{ sourceId:'mutar-nomoreflat', title:'safe-title', apiKey:'super-secret-value', summary:{ token:'nested-secret' } }] };
  const workspace = createVrResearchQaWorkspaceAnswerRecord(q, forged, { proofVerifier, validationOptions:{ nowMs } });
  assert.equal(workspace.validation.valid, true, workspace.validation.errors.join(', '));
  const body = JSON.parse(workspace.record.body);
  assert.deepEqual(body.answer.facts, [{ sourceId:'mutar-nomoreflat', title:'safe-title' }]);
  assert.equal(workspace.record.body.includes('super-secret-value'), false); assert.equal(workspace.record.body.includes('nested-secret'), false);
});
...
test('canonical source and experiment facts survive Workspace fact sanitization', () => {
  for (const [questionClass,index] of [['SOURCE_STACK',0],['NEXT_EXPERIMENT',1]]) {
    const q = request(questionClass,index); const answered = answerVrResearchQuestion(q, projection(), qaInput());
    const workspace = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { proofVerifier, validationOptions:{ nowMs } });
    assert.equal(workspace.validation.valid, true, workspace.validation.errors.join(', '));
...
test('default Workspace correlation safely derives long colon-bearing question ids', () => {
  const q = request('SOURCE_STACK',0,{ questionId:`vr:${'a'.repeat(100)}` }); const answered = answerVrResearchQuestion(q, projection(), qaInput());
  const workspace = createVrResearchQaWorkspaceAnswerRecord(q, answered.answer, { proofVerifier, validationOptions:{ nowMs } });
  assert.equal(workspace.validation.valid, true, workspace.validation.errors.join(', '));
  assert.match(workspace.record.correlationId, /^vr-correlation-[0-9a-f]{24}$/); assert.equal(workspace.record.correlationId.includes(':'), false);
});
