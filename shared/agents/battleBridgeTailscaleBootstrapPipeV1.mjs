...
export function buildFixedBattleBridgeBootstrapPowerShell(expectedHead) {
  ...
  return [
    ...
    "$installerOutput = @(& $installer -StartNow)",
    "if ($LASTEXITCODE -ne 0) { throw 'TAILSCALE_BOOTSTRAP_SYNC_INSTALLER_FAILED' }",
    ...
    "$statusOutput = @(& $statusScript)",
    "if ($LASTEXITCODE -ne 0) { throw 'TAILSCALE_BOOTSTRAP_SYNC_STATUS_FAILED' }",
