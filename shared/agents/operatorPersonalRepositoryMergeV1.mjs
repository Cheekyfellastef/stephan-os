...
export function extractPersonalRepositoryArtifactZip(archiveBytes, expectedFileName) {
  if (!(Buffer.isBuffer(archiveBytes) || archiveBytes instanceof Uint8Array)) invalidArtifactZip('archive-type');
  if (typeof expectedFileName !== 'string'
    || expectedFileName.length === 0
    || expectedFileName.length > 255
    || !/^[A-Za-z0-9._-]+$/.test(expectedFileName)) invalidArtifactZip('expected-filename');
  const archive = Buffer.from(archiveBytes);
  if (archive.length < ZIP_LOCAL_FILE_HEADER_BYTES + ZIP_CENTRAL_DIRECTORY_HEADER_BYTES + ZIP_END_OF_CENTRAL_DIRECTORY_BYTES
    || archive.length > PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_MAX_BYTES) invalidArtifactZip('archive-size');

  const endOffset = archive.length - ZIP_END_OF_CENTRAL_DIRECTORY_BYTES;
  if (uint32(archive, endOffset, 'end-record') !== ZIP_END_OF_CENTRAL_DIRECTORY_SIGNATURE) invalidArtifactZip('end-signature');
  ...
  if (centralOffset < ZIP_LOCAL_FILE_HEADER_BYTES
    ...
    || centralOffset + centralSize !== endOffset) invalidArtifactZip('central-boundary');

  if (uint32(archive, centralOffset, 'central-record') !== ZIP_CENTRAL_DIRECTORY_HEADER_SIGNATURE) invalidArtifactZip('central-signature');
  ...
  const centralRecordBytes = ZIP_CENTRAL_DIRECTORY_HEADER_BYTES + centralNameLength + centralExtraLength + centralCommentLength;
  ...
  if (centralUncompressedSize > PERSONAL_REPOSITORY_ARTIFACT_PAYLOAD_MAX_BYTES) invalidArtifactZip('payload-size');
  if (centralNameLength === 0 || centralExtraLength !== 0 || centralCommentLength !== 0) invalidArtifactZip('central-fields');
  ...
  if (uint32(archive, 0, 'local-record') !== ZIP_LOCAL_FILE_HEADER_SIGNATURE) invalidArtifactZip('local-signature');
  ...
  if (localVersionNeeded !== centralVersionNeeded
    ...
    || localExtraLength !== 0) invalidArtifactZip('local-central-mismatch');
  if (usesDataDescriptor) {
    if (localCrc32 !== 0 || localCompressedSize !== 0 || localUncompressedSize !== 0) invalidArtifactZip('descriptor-local-fields');
  } else if (localCrc32 !== centralCrc32
    || localCompressedSize !== centralCompressedSize
    || localUncompressedSize !== centralUncompressedSize) invalidArtifactZip('local-central-mismatch');
  ...
  if (usesDataDescriptor) {
    ...
    if (uint32(archive, dataEnd, 'descriptor') !== ZIP_DATA_DESCRIPTOR_SIGNATURE) invalidArtifactZip('descriptor-signature');
    if (uint32(archive, dataEnd + 4, 'descriptor') !== centralCrc32
      || uint32(archive, dataEnd + 8, 'descriptor') !== centralCompressedSize
      || uint32(archive, dataEnd + 12, 'descriptor') !== centralUncompressedSize) invalidArtifactZip('descriptor-mismatch');
  } else if (dataEnd !== centralOffset) invalidArtifactZip('record-overlap-or-gap');
  if (localMethod === ZIP_STORED_METHOD && centralCompressedSize !== centralUncompressedSize) invalidArtifactZip('stored-size');
  ...
  if (payload.length !== centralUncompressedSize || payload.length > PERSONAL_REPOSITORY_ARTIFACT_PAYLOAD_MAX_BYTES) invalidArtifactZip('payload-size-mismatch');
...
export class PersonalRepositoryReadPolicyError extends Error {
  constructor(path, blockers = []) {
    const endpoint = text(path).replace(/[\r\n\t]/g, '').slice(0, 500) || 'unknown-endpoint';
    const boundedBlockers = (Array.isArray(blockers) ? blockers : []).map(text).filter(Boolean).slice(0, 10);
...
export async function readBoundedPersonalRepositoryResponseBody(response, maxBytes) {
  ...
  if (!boundedMax || !reader || typeof reader.read !== 'function') {
    ...
    throw new PersonalRepositoryReadPolicyError('artifact-archive-body', ['personal-repository-artifact-archive-body-not-readable']);
  }
  const chunks = [];
  let totalBytes = 0;
  try {
    while (true) {
      ...
      if (!(result.value instanceof Uint8Array) || result.value.length < 1) throw new PersonalRepositoryReadPolicyError('artifact-archive-body', ['personal-repository-artifact-archive-body-chunk-invalid']);
      totalBytes += result.value.length;
      if (!Number.isSafeInteger(totalBytes) || totalBytes > boundedMax) throw new PersonalRepositoryReadPolicyError('artifact-archive-body', ['personal-repository-artifact-archive-body-size-exceeded']);
  ...
  if (totalBytes < 1) throw new PersonalRepositoryReadPolicyError('artifact-archive-body', ['personal-repository-artifact-archive-body-empty']);
...
export async function executeBoundedPersonalRepositoryRead({
  ...
  if (validateResponse !== null && typeof validateResponse !== 'function') throw new TypeError('Personal-repository response validator must be a function when supplied.');
  ...
  for (let attempt = 1; attempt <= maximumAttempts; attempt += 1) {
    try {
      ...
      if (transientStatus && attempt < maximumAttempts) await response?.body?.cancel?.();
      else {
        if (validateResponse) {
      ...
      if (attempt === maximumAttempts) throw new PersonalRepositoryReadTransportError(path, attempt, error);
    }
...
export function validatePersonalRepositoryRulesetProofRequest(input = {}) {
  ...
  if (!REPOSITORY_PATTERN.test(repository) || !pathRepository || `${pathRepository[1]}/${pathRepository[2]}` !== repository) blockers.push('personal-repository-ruleset-proof-repository-mismatch');
  if (!PERSONAL_REPOSITORY_RULESET_PROOF_PATHS.some((pattern) => pattern.test(path))) blockers.push('personal-repository-ruleset-proof-path-not-bounded');
  return Object.freeze({ valid: blockers.length === 0, blockers: Object.freeze(blockers), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_RULESET_PROOF_REQUEST_BLOCKED' : 'PERSONAL_REPOSITORY_RULESET_PROOF_REQUEST_READY' });
}
...
export function validatePersonalRepositoryRulesetProofResponse(input = {}) {
  ...
  const blockers = [];
  if (response?.redirected !== false) blockers.push('personal-repository-ruleset-proof-response-redirected');
  if (responseUrl !== expectedUrl) blockers.push('personal-repository-ruleset-proof-response-url-mismatch');
  return Object.freeze({ valid: blockers.length === 0, blockers: Object.freeze(blockers), expectedUrl, finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_RULESET_PROOF_RESPONSE_BLOCKED' : 'PERSONAL_REPOSITORY_RULESET_PROOF_RESPONSE_READY' });
}
...
function artifactArchiveUrlBlockers(location) {
  ...
  if (url.protocol !== 'https:' || (url.port && url.port !== '443')) blockers.push('personal-repository-artifact-archive-scheme-invalid');
  if (url.username || url.password) blockers.push('personal-repository-artifact-archive-credentials-present');
  if (!PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_HOST.test(url.hostname)) blockers.push('personal-repository-artifact-archive-host-not-allowlisted');
  if (!PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_PATH.test(url.pathname)) blockers.push('personal-repository-artifact-archive-path-not-bounded');
  ...
  if (observedKeys.length !== expectedKeys.length || observedKeys.some((key, index) => key !== expectedKeys[index]) || observedKeys.some((key) => url.searchParams.getAll(key).length !== 1 || !url.searchParams.get(key) || url.searchParams.get(key).length > 1024)) blockers.push('personal-repository-artifact-archive-query-invalid');
  if (url.searchParams.get('spr') !== 'https' || url.searchParams.get('sp') !== 'r' || url.searchParams.get('sr') !== 'b') blockers.push('personal-repository-artifact-archive-scope-invalid');
  return blockers;
}
...
export function buildPersonalRepositoryArtifactApiRequest(input = {}) {
  ...
  const blockers = [];
  if (!pathMatch || `${pathMatch?.[1]}/${pathMatch?.[2]}` !== repository) blockers.push('personal-repository-artifact-api-path-mismatch');
  return Object.freeze({ valid: blockers.length === 0, blockers: Object.freeze(blockers), request: blockers.length ? null : Object.freeze({ url: `${PERSONAL_REPOSITORY_GITHUB_API_ORIGIN}${path}`, method: 'GET', body: null, redirect: 'manual', headers: Object.freeze({ Accept: 'application/vnd.github+json' }) }), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_ARTIFACT_API_REQUEST_BLOCKED' : 'PERSONAL_REPOSITORY_ARTIFACT_API_REQUEST_READY' });
}
...
export function validatePersonalRepositoryArtifactArchiveRedirect(input = {}) {
  ...
  if (typeof response?.url !== 'string' || response.url !== expectedUrl) blockers.push('personal-repository-artifact-api-response-url-mismatch');
  blockers.push(...artifactArchiveUrlBlockers(location));
  return Object.freeze({ valid: blockers.length === 0, blockers: Object.freeze(blockers), location: blockers.length ? '' : location, finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_REDIRECT_BLOCKED' : 'PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_REDIRECT_READY' });
}

export function buildPersonalRepositoryArtifactArchiveRequest(location) {
  const blockers = artifactArchiveUrlBlockers(location);
  if (blockers.length) return Object.freeze({ valid: false, blockers: Object.freeze(blockers), request: null, finalVerdict: 'PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_REQUEST_BLOCKED' });
  const url = artifactArchiveUrl(location).toString();
  return Object.freeze({ valid: true, blockers: Object.freeze([]), request: Object.freeze({ url, method: 'GET', body: null, redirect: 'manual', headers: Object.freeze({ Accept: 'application/zip' }) }), finalVerdict: 'PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_REQUEST_READY' });
}
...
export function validatePersonalRepositoryArtifactArchiveResponse(input = {}) {
  ...
  if (typeof response?.url !== 'string' || response.url !== expectedUrl) blockers.push('personal-repository-artifact-archive-response-url-mismatch');
  if (contentType !== 'application/zip') blockers.push('personal-repository-artifact-archive-content-type-invalid');
  if (!maxBytes || !Number.isSafeInteger(contentLength) || contentLength < 1 || contentLength > maxBytes) blockers.push('personal-repository-artifact-archive-content-length-invalid');
  return Object.freeze({ valid: blockers.length === 0, blockers: Object.freeze(blockers), contentLength: blockers.length ? 0 : contentLength, finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_RESPONSE_BLOCKED' : 'PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_RESPONSE_READY' });
}
...
export async function executePersonalRepositoryArtifactArchiveTransport(input = {}) {
  ...
  if (!maxBytes || maxBytes > PERSONAL_REPOSITORY_ARTIFACT_ARCHIVE_MAX_BYTES) inputBlockers.push('personal-repository-artifact-archive-max-bytes-invalid');
  if (typeof requestApiRedirect !== 'function') inputBlockers.push('personal-repository-artifact-api-request-function-invalid');
  if (typeof requestArchive !== 'function') inputBlockers.push('personal-repository-artifact-archive-request-function-invalid');
  if (input.delay !== undefined && typeof input.delay !== 'function') inputBlockers.push('personal-repository-artifact-delay-function-invalid');
  if (inputBlockers.length) throw new PersonalRepositoryReadPolicyError(path, inputBlockers);

  const { response: redirectResponse } = await executeBoundedPersonalRepositoryRead({ path, request: () => requestApiRedirect(apiRequest.request), validateResponse: (response) => validatePersonalRepositoryArtifactArchiveRedirect({ path, repository, response }), ...delayOptions });
  const download = buildPersonalRepositoryArtifactArchiveRequest(responseHeader(redirectResponse, 'location'));
  ...
  const { result: bytes } = await executeBoundedPersonalRepositoryRead({
    ...
    validateResponse: (response) => {
      const validation = validatePersonalRepositoryArtifactArchiveResponse({ expectedUrl: download.request.url, response, maxBytes });
  ...
  if (!Buffer.isBuffer(bytes) || bytes.length < 1 || bytes.length > maxBytes || bytes.length !== declaredContentLength) throw new PersonalRepositoryReadPolicyError(path, ['personal-repository-artifact-archive-body-length-mismatch']);
...
function canonicalConfigurationValue(value) {
  ...
  if (value && typeof value === 'object') return Object.fromEntries(Object.keys(value).sort().map((key) => [key, canonicalConfigurationValue(value[key])]));
  return value;
}
...
export function buildPersonalRepositoryConfigurationEvidence(input = {}) {
  ...
  const activeRules = (Array.isArray(input.activeRules) ? input.activeRules : []).map(canonicalConfigurationValue).sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
  const rulesets = (Array.isArray(input.rulesets) ? input.rulesets : []).map((ruleset) => canonicalConfigurationValue({ id: ruleset?.id, name: ruleset?.name, target: ruleset?.target, source_type: ruleset?.source_type, source: ruleset?.source, enforcement: ruleset?.enforcement, created_at: ruleset?.created_at, updated_at: ruleset?.updated_at, conditions: ruleset?.conditions, rules: ruleset?.rules, bypass_actors: ruleset?.bypass_actors })).sort((left, right) => Number(left.id) - Number(right.id));
  return Object.freeze(canonicalConfigurationValue({ repository: { id: repository?.id, owner_type: repository?.owner?.type, private: repository?.private, visibility: repository?.visibility, default_branch: repository?.default_branch, allow_squash_merge: repository?.allow_squash_merge, delete_branch_on_merge: repository?.delete_branch_on_merge }, environment: { name: environment?.name, can_admins_bypass: environment?.can_admins_bypass, deployment_branch_policy: environment?.deployment_branch_policy, protection_rules: environment?.protection_rules }, activeRules, rulesets }));
}

function workflowRepository(run = {}) { return text(run?.repository?.full_name || run?.repository); }
...
function canonicalWorkflowPath(run = {}, repository = '') {
  ...
  const allowed = new Set([text(pullRequest?.head?.ref), text(pullRequest?.base?.ref), text(pullRequest?.head?.ref) ? `refs/heads/${text(pullRequest.head.ref)}` : '', text(pullRequest?.base?.ref) ? `refs/heads/${text(pullRequest.base.ref)}` : '', strictPositiveInteger(pullRequest?.number) ? `refs/pull/${pullRequest.number}/merge` : ''].filter(Boolean));
...
function personalRepositoryDispatchActor(run = {}) { return text(run?.triggering_actor?.login || run?.actor?.login).toLowerCase(); }
function personalRepositoryDispatchTitle(sourceHead = '') { return `Protected operator merge ${text(sourceHead).toLowerCase()}`; }
...
export function validatePersonalRepositoryDispatchWorkflowDefinition(definitions = []) {
  ...
  const matches = (Array.isArray(definitions) ? definitions : []).filter((definition) => text(definition?.path) === PERSONAL_REPOSITORY_WORKFLOW_PATH);
  const definition = matches[0];
  if (matches.length !== 1 || definition?.name !== PERSONAL_REPOSITORY_WORKFLOW_NAME || definition?.state !== 'active' || !strictPositiveInteger(definition?.id)) blockers.push('personal-repository-workflow-definition-not-exact');
  const valid = blockers.length === 0;
  return Object.freeze({ valid, definition: valid ? Object.freeze({ id: definition.id, name: definition.name, path: definition.path, state: definition.state }) : null, blockers: Object.freeze(unique(blockers)), finalVerdict: valid ? 'PERSONAL_REPOSITORY_DISPATCH_WORKFLOW_DEFINITION_READY' : 'PERSONAL_REPOSITORY_DISPATCH_WORKFLOW_DEFINITION_BLOCKED' });
}
...
export function validatePersonalRepositoryDispatchExecution(input = {}, expected = {}) {
  ...
  const expectedActor = mailboxAuthorizationValid ? 'github-actions[bot]' : nativeOwnerActor;
  ...
  if (workflowRunId && workflowRunAttempt > 1 && currentMismatches.length === 0) replayRunIds.push(workflowRunId);
  for (const candidate of priorRuns) {
    ...
    const sourceMatching = text(candidate?.name) === expectedTitle || text(candidate?.display_title) === expectedTitle;
    ...
    const exactIdentity = Boolean(candidateId
    ...
    if (!exactIdentity) { malformedPriorRunIds.push(candidateId || 0); continue; }
    if (candidateBase === baseSha) {
      sameBasePriorAttemptCount = Math.min(PERSONAL_REPOSITORY_PRIOR_ATTEMPT_JOB_PROOF_MAX + 1, sameBasePriorAttemptCount + Math.min(strictPositiveInteger(candidate?.run_attempt), PERSONAL_REPOSITORY_PRIOR_ATTEMPT_JOB_PROOF_MAX + 1));
      const matchingJobSets = priorRunJobSets.filter((item) => strictPositiveInteger(item?.runId) === candidateId);
      const retryValidation = matchingJobSets.length === 1 ? validatePersonalRepositoryReadOnlyPriorFailure(candidate, matchingJobSets[0]?.jobs) : { valid: false };
      if (retryValidation.valid) { retryablePriorRunIds.push(candidateId); retryablePriorFailures.push(retryValidation.receipt); }
  ...
  const retryableProofDuplicate = new Set(retryablePriorRunIds).size !== retryablePriorRunIds.length || new Set(retryableJobIds).size !== retryableJobIds.length;
  ...
  const blockers = [
    ...
    ...(priorRunJobSets.length > PERSONAL_REPOSITORY_PRIOR_ATTEMPT_JOB_PROOF_MAX ? ['personal-repository-prior-run-jobs-limit-exceeded'] : []),
    ...
    ...(mailboxAuthorizationSupplied && !mailboxAuthorizationValid ? ['personal-repository-mailbox-authorization-provenance-invalid'] : []),
  ...
  return Object.freeze({
    ...
    blockers: Object.freeze(unique(blockers)),
    finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_DISPATCH_EXECUTION_BLOCKED' : 'PERSONAL_REPOSITORY_DISPATCH_EXECUTION_READY',
  });
}
...
export function parsePersonalRepositoryDispatchInputs(inputs = {}) {
  ...
  for (const [key, blocker] of [['sourceHead', 'personal-repository-head-invalid'], ['sourceTree', 'personal-repository-tree-invalid'], ['baseSha', 'personal-repository-base-invalid']]) if (!SHA_PATTERN.test(parsed[key])) blockers.push(blocker);
  ...
  if (!ARTIFACT_DIGEST_PATTERN.test(parsed.independentReviewArtifactDigest)) blockers.push('personal-repository-review-artifact-digest-invalid');
  if (!SHA256_PATTERN.test(parsed.independentReviewPayloadSha256)) blockers.push('personal-repository-review-payload-digest-invalid');
  return Object.freeze({ valid: blockers.length === 0, identity: parsed, blockers: Object.freeze(blockers), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_DISPATCH_BLOCKED' : 'PERSONAL_REPOSITORY_DISPATCH_READY' });
}
...
export function validatePersonalRepositoryWorkflowRuns(definitions = [], runs = [], expected = {}) {
  ...
  for (const required of PERSONAL_REPOSITORY_REQUIRED_WORKFLOWS) {
    const matches = (Array.isArray(definitions) ? definitions : []).filter((definition) => text(definition?.path) === required.path);
    const definition = matches[0];
    if (matches.length !== 1 || definition?.name !== required.name || definition?.state !== 'active' || !strictPositiveInteger(definition?.id)) { blockers.push(`personal-repository-workflow-definition-not-exact:${required.name}`); continue; }
    const candidates = (Array.isArray(runs) ? runs : []).filter((run) => strictPositiveInteger(run?.workflow_id) === definition.id && text(run?.name) === required.name && canonicalWorkflowPath(run, repository) === required.path && text(run?.event) === required.event && workflowRepository(run) === repository && text(run?.head_sha).toLowerCase() === sourceHead).sort((left, right) => strictPositiveInteger(right?.run_number) - strictPositiveInteger(left?.run_number) || strictPositiveInteger(right?.id) - strictPositiveInteger(left?.id));
    const run = candidates[0];
    const bindings = Array.isArray(run?.pull_requests) ? run.pull_requests : [];
    const binding = bindings.length === 1 ? bindings[0] : null;
    if (!run || text(run?.status).toLowerCase() !== 'completed' || text(run?.conclusion).toLowerCase() !== 'success' || !strictPositiveInteger(run?.id) || !strictPositiveInteger(run?.run_attempt) || bindings.length !== 1 || strictPositiveInteger(binding?.number) !== prNumber || text(binding?.head?.sha).toLowerCase() !== sourceHead || text(binding?.head?.ref) !== branch || text(binding?.base?.sha).toLowerCase() !== baseSha || text(binding?.base?.ref) !== 'main') { blockers.push(`personal-repository-workflow-run-not-exact-green:${required.name}`); continue; }
    evidence.push(Object.freeze({ name: required.name, path: required.path, workflowId: definition.id, runId: run.id, runAttempt: run.run_attempt, checkSuiteId: strictPositiveInteger(run?.check_suite_id) || null }));
  }
  return Object.freeze({ valid: blockers.length === 0 && evidence.length === PERSONAL_REPOSITORY_REQUIRED_WORKFLOWS.length, evidence: Object.freeze(evidence), blockers: Object.freeze(unique(blockers)), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_WORKFLOWS_BLOCKED' : 'PERSONAL_REPOSITORY_WORKFLOWS_READY' });
}

export function validatePersonalRepositoryWorkflowRunHydration(summaries = [], details = [], expected = {}) {
  const blockers = [];
  const sourceHead = text(expected.sourceHead).toLowerCase();
  if (!Array.isArray(summaries) || summaries.length === 0) blockers.push('personal-repository-workflow-run-summaries-invalid');
  if (!Array.isArray(details) || details.length === 0) blockers.push('personal-repository-workflow-run-details-invalid');
  if (!SHA_PATTERN.test(sourceHead)) blockers.push('personal-repository-workflow-run-hydration-head-invalid');
  const summaryIds = new Set();
  const detailById = new Map();
  for (const detail of Array.isArray(details) ? details : []) {
    const detailId = strictPositiveInteger(detail?.id);
    if (!detailId || detailById.has(detailId)) { blockers.push('personal-repository-workflow-run-detail-identity-invalid'); continue; }
  ...
  for (const summary of Array.isArray(summaries) ? summaries : []) {
    ...
    if (!summaryId || !workflowId || !checkSuiteId || text(summary?.head_sha).toLowerCase() !== sourceHead || summaryIds.has(summaryId)) { blockers.push('personal-repository-workflow-run-summary-identity-invalid'); continue; }
    summaryIds.add(summaryId);
    const detail = detailById.get(summaryId);
    if (!detail || strictPositiveInteger(detail?.workflow_id) !== workflowId || strictPositiveInteger(detail?.check_suite_id) !== checkSuiteId || text(detail?.head_sha).toLowerCase() !== sourceHead) { blockers.push('personal-repository-workflow-run-detail-mismatch'); continue; }
    hydratedRuns.push(detail);
  }
  if (summaryIds.size !== detailById.size) blockers.push('personal-repository-workflow-run-hydration-cardinality-mismatch');
  const valid = blockers.length === 0 && hydratedRuns.length === summaryIds.size;
  return Object.freeze({ valid, runs: valid ? Object.freeze([...hydratedRuns]) : Object.freeze([]), blockers: Object.freeze(unique(blockers)), finalVerdict: valid ? 'PERSONAL_REPOSITORY_WORKFLOW_RUN_HYDRATION_READY' : 'PERSONAL_REPOSITORY_WORKFLOW_RUN_HYDRATION_BLOCKED' });
}

export function buildPersonalRepositoryCheckExpectation({ repository = '', identity = {}, mergeStateStatus = '' } = {}) {
  const expected = Object.freeze({ repository: text(repository), prNumber: strictPositiveInteger(identity?.prNumber), branch: text(identity?.branch), sourceHead: text(identity?.sourceHead).toLowerCase(), baseSha: text(identity?.baseSha).toLowerCase(), mergeStateStatus: text(mergeStateStatus).toUpperCase() });
  const blockers = [];
  if (!REPOSITORY_PATTERN.test(expected.repository)) blockers.push('personal-repository-check-expectation-repository-invalid');
  if (!expected.prNumber) blockers.push('personal-repository-check-expectation-pr-invalid');
  if (!BRANCH_PATTERN.test(expected.branch) || expected.branch.includes('..')) blockers.push('personal-repository-check-expectation-branch-invalid');
  if (!SHA_PATTERN.test(expected.sourceHead)) blockers.push('personal-repository-check-expectation-head-invalid');
  if (!SHA_PATTERN.test(expected.baseSha)) blockers.push('personal-repository-check-expectation-base-invalid');
  if (!['CLEAN', 'UNSTABLE'].includes(expected.mergeStateStatus)) blockers.push('personal-repository-check-expectation-merge-state-invalid');
  return Object.freeze({ valid: blockers.length === 0, expected: blockers.length === 0 ? expected : null, blockers: Object.freeze(blockers), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_CHECK_EXPECTATION_BLOCKED' : 'PERSONAL_REPOSITORY_CHECK_EXPECTATION_READY' });
}

export function validatePersonalRepositoryCheckRuns(checkRuns = [], workflowRuns = [], commitStatuses = [], expected = {}, options = {}) {
  const blockers = [];
  const evidence = [];
  const repository = text(expected.repository);
  const prNumber = strictPositiveInteger(expected.prNumber);
  ...
  if (!Array.isArray(checkRuns) || checkRuns.length === 0) blockers.push('personal-repository-check-runs-invalid');
  ...
  const exactCheckBindings = (Array.isArray(checkRuns) ? checkRuns : []).map((check) => {
    ...
    const matchingRuns = (Array.isArray(workflowRuns) ? workflowRuns : []).filter((run) => strictPositiveInteger(run?.check_suite_id) === checkSuiteId && text(run?.head_sha).toLowerCase() === sourceHead);
    const run = matchingRuns[0];
    const bindings = Array.isArray(run?.pull_requests) ? run.pull_requests : [];
    const binding = bindings.length === 1 ? bindings[0] : null;
    const detailsUrl = `https://github.com/${repository}/actions/runs/${run?.id}/job/${checkId}`;
    const exactRun = matchingRuns.length === 1 && strictPositiveInteger(run?.id) && strictPositiveInteger(run?.run_attempt) && workflowRepository(run) === repository && bindings.length === 1 && strictPositiveInteger(binding?.number) === prNumber && text(binding?.head?.sha).toLowerCase() === sourceHead && text(binding?.head?.ref) === branch && text(binding?.base?.sha).toLowerCase() === baseSha && text(binding?.base?.ref) === 'main' && text(check?.details_url) === detailsUrl;
    return Object.freeze({ check, checkId, checkSuiteId, matchingRuns, run, exactRun });
  });
  for (const status of Array.isArray(commitStatuses) ? commitStatuses : []) if (text(status?.sha).toLowerCase() !== sourceHead || text(status?.state).toLowerCase() !== 'success') blockers.push('personal-repository-commit-status-not-exact-green');
  for (const checkBinding of exactCheckBindings) {
    const { check, checkId, checkSuiteId, matchingRuns, run, exactRun } = checkBinding;
    const name = text(check?.name), status = text(check?.status).toLowerCase(), conclusion = text(check?.conclusion).toLowerCase(), workflow = text(run?.name), path = canonicalWorkflowPath(run, repository);
    if (!checkId || !checkSuiteId || !name || text(check?.head_sha).toLowerCase() !== sourceHead || text(check?.app?.slug) !== 'github-actions' || strictPositiveInteger(check?.app?.id) !== 15368) { blockers.push('personal-repository-check-run-identity-invalid'); continue; }
    if (matchingRuns.length === 0) { blockers.push('personal-repository-check-workflow-run-missing'); continue; }
    if (!exactRun) { blockers.push('personal-repository-check-run-identity-invalid'); continue; }
    const supersededDraftSkip = conclusion === 'skipped' && workflow === PERSONAL_REPOSITORY_REVIEW_ESCALATION.workflow && path === PERSONAL_REPOSITORY_REVIEW_ESCALATION.path && text(run?.event) === PERSONAL_REPOSITORY_REVIEW_ESCALATION.event && name === PERSONAL_REPOSITORY_REVIEW_ESCALATION.check && exactCheckBindings.some((candidate) => candidate !== checkBinding && candidate.exactRun && strictPositiveInteger(candidate.run?.id) > strictPositiveInteger(run?.id) && text(candidate.check?.head_sha).toLowerCase() === sourceHead && text(candidate.check?.status).toLowerCase() === 'completed' && text(candidate.check?.conclusion).toLowerCase() === 'success' && text(candidate.check?.name) === PERSONAL_REPOSITORY_REVIEW_ESCALATION.check && text(candidate.check?.app?.slug) === 'github-actions' && strictPositiveInteger(candidate.check?.app?.id) === 15368 && text(candidate.run?.name) === PERSONAL_REPOSITORY_REVIEW_ESCALATION.workflow && canonicalWorkflowPath(candidate.run, repository) === PERSONAL_REPOSITORY_REVIEW_ESCALATION.path && text(candidate.run?.event) === PERSONAL_REPOSITORY_REVIEW_ESCALATION.event);
    let disposition = 'green';
    if (status !== 'completed') { blockers.push('personal-repository-check-run-not-terminal'); disposition = 'blocked'; }
    else if (conclusion === 'success') {}
    else if (conclusion === 'skipped' && PERSONAL_REPOSITORY_NEUTRAL_SKIPPED_CHECKS.has(`${workflow}\0${name}`)) disposition = 'neutral-skip';
    else if (supersededDraftSkip) disposition = 'superseded-draft-skip';
    else if (cleanIndependentReviewProved && mergeStateStatus === 'UNSTABLE' && workflow === PERSONAL_REPOSITORY_REVIEW_ESCALATION.workflow && path === PERSONAL_REPOSITORY_REVIEW_ESCALATION.path && text(run?.event) === PERSONAL_REPOSITORY_REVIEW_ESCALATION.event && name === PERSONAL_REPOSITORY_REVIEW_ESCALATION.check && conclusion === PERSONAL_REPOSITORY_REVIEW_ESCALATION.conclusion) { admittedReviewEscalations += 1; disposition = 'clean-independent-review'; }
    else { blockers.push('personal-repository-check-run-not-exact-green'); disposition = 'blocked'; }
    evidence.push(Object.freeze({ checkId, checkSuiteId, name, workflow, path, workflowRunId: run.id, workflowRunAttempt: run.run_attempt, status, conclusion, disposition }));
  }
  if (mergeStateStatus === 'UNSTABLE' && admittedReviewEscalations !== 1) blockers.push('personal-repository-review-escalation-check-not-exact');
  if (mergeStateStatus === 'CLEAN' && admittedReviewEscalations !== 0) blockers.push('personal-repository-clean-state-has-review-escalation');
  return Object.freeze({ valid: blockers.length === 0, evidence: Object.freeze(evidence), admittedReviewEscalations, blockers: Object.freeze(unique(blockers)), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_CHECK_RUNS_BLOCKED' : 'PERSONAL_REPOSITORY_CHECK_RUNS_READY' });
}

export async function validatePersonalRepositoryCheckRunsWithBoundedReread({ readSnapshot, waitBeforeReread = (delayMs) => new Promise((resolve) => setTimeout(resolve, delayMs)), monotonicNow = () => performance.now(), expected = {}, options = {} } = {}) {
  if (typeof readSnapshot !== 'function' || typeof waitBeforeReread !== 'function' || typeof monotonicNow !== 'function') return Object.freeze({ valid: false, evidence: Object.freeze([]), admittedReviewEscalations: 0, blockers: Object.freeze(['personal-repository-check-snapshot-reader-invalid']), snapshotAttempt: 0, snapshotAttempts: Object.freeze([]), selectedSnapshot: null, convergenceDeadlineReached: false, finalVerdict: 'PERSONAL_REPOSITORY_CHECK_RUNS_BLOCKED' });
  const snapshotAttempts = [];
  const startedAtMs = monotonicNow();
  const deadlineMs = startedAtMs + PERSONAL_REPOSITORY_CHECK_SNAPSHOT_CONVERGENCE_TIMEOUT_MS;
  if (!Number.isFinite(startedAtMs)) return Object.freeze({ valid: false, evidence: Object.freeze([]), admittedReviewEscalations: 0, blockers: Object.freeze(['personal-repository-check-snapshot-clock-invalid']), snapshotAttempt: 0, snapshotAttempts: Object.freeze([]), selectedSnapshot: null, convergenceDeadlineReached: false, finalVerdict: 'PERSONAL_REPOSITORY_CHECK_RUNS_BLOCKED' });
  let validation = null;
  let attempt = 0;
  while (true) {
    attempt += 1;
    const snapshot = await readSnapshot(attempt);
    validation = validatePersonalRepositoryCheckRuns(snapshot?.checkRuns, snapshot?.workflowRuns, snapshot?.commitStatuses, expected, options);
    const retryable = validation.blockers.length > 0 && validation.blockers.every((blocker) => PERSONAL_REPOSITORY_CHECK_SNAPSHOT_TRANSIENT_BLOCKERS.has(blocker));
    snapshotAttempts.push(Object.freeze({ attempt, valid: validation.valid, retryable, blockers: validation.blockers }));
    if (validation.valid) return Object.freeze({ ...validation, snapshotAttempt: attempt, snapshotAttempts: Object.freeze(snapshotAttempts), selectedSnapshot: Object.freeze({ checkRuns: Object.freeze([...snapshot.checkRuns]), workflowRuns: Object.freeze([...snapshot.workflowRuns]), commitStatuses: Object.freeze([...snapshot.commitStatuses]) }), convergenceDeadlineReached: false });
    const beforeWaitMs = monotonicNow();
    const remainingMs = deadlineMs - beforeWaitMs;
    if (!retryable || !Number.isFinite(beforeWaitMs) || remainingMs <= 0) return Object.freeze({ ...validation, snapshotAttempt: 0, snapshotAttempts: Object.freeze(snapshotAttempts), selectedSnapshot: null, convergenceDeadlineReached: retryable && Number.isFinite(beforeWaitMs) && remainingMs <= 0 });
    await waitBeforeReread(Math.min(PERSONAL_REPOSITORY_CHECK_SNAPSHOT_POLL_INTERVAL_MS, remainingMs));
    const afterWaitMs = monotonicNow();
    if (!Number.isFinite(afterWaitMs) || afterWaitMs <= beforeWaitMs) return Object.freeze({ ...validation, valid: false, blockers: Object.freeze(unique([...validation.blockers, 'personal-repository-check-snapshot-clock-invalid'])), snapshotAttempt: 0, snapshotAttempts: Object.freeze(snapshotAttempts), selectedSnapshot: null, convergenceDeadlineReached: false, finalVerdict: 'PERSONAL_REPOSITORY_CHECK_RUNS_BLOCKED' });
  }
}
...
export function validatePersonalRepositoryEvidence(input = {}, expected = {}, options = {}) {
  ...
  if (text(input.triggeringActor).toLowerCase() !== OPERATOR_MERGE_REVIEWER.toLowerCase()) blockers.push('personal-repository-triggering-actor-not-operator');
  ...
  if (reviewDecision === 'CHANGES_REQUESTED') blockers.push('personal-repository-changes-requested'); else if (!['', 'APPROVED'].includes(reviewDecision)) blockers.push('personal-repository-review-decision-unsupported');
  if (mergeable !== 'MERGEABLE') blockers.push('personal-repository-pr-not-mergeable');
  if (mergeStateStatus !== 'CLEAN' && !(mergeStateStatus === 'UNSTABLE' && cleanIndependentReviewProved && reviewEscalationChecksProved)) blockers.push('personal-repository-pr-not-clean');
  if (!Number.isSafeInteger(input.unresolvedThreadCount) || input.unresolvedThreadCount !== 0) blockers.push('personal-repository-conversations-not-resolved');
  if (text(comparison.status).toLowerCase() !== 'ahead' || !Number.isSafeInteger(comparison.ahead_by) || comparison.ahead_by < 1 || comparison.behind_by !== 0 || text(comparison?.base_commit?.sha).toLowerCase() !== baseSha || text(comparison?.merge_base_commit?.sha).toLowerCase() !== baseSha) blockers.push('personal-repository-comparison-not-exact-forward');
  for (const [key, observed, normalize, blocker] of [['repository', repository, text(expected.repository), 'personal-repository-expected-repository-mismatch'], ['prNumber', prNumber, strictPositiveInteger(expected.prNumber), 'personal-repository-expected-pr-mismatch'], ['branch', branch, text(expected.branch), 'personal-repository-expected-branch-mismatch'], ['sourceHead', sourceHead, text(expected.sourceHead).toLowerCase(), 'personal-repository-expected-head-mismatch'], ['sourceTree', sourceTree, text(expected.sourceTree).toLowerCase(), 'personal-repository-expected-tree-mismatch'], ['baseSha', baseSha, text(expected.baseSha).toLowerCase(), 'personal-repository-expected-base-mismatch'], ['workflowRunId', workflowRunId, strictPositiveInteger(expected.workflowRunId), 'personal-repository-expected-run-mismatch'], ['workflowRunAttempt', workflowRunAttempt, strictPositiveInteger(expected.workflowRunAttempt), 'personal-repository-expected-attempt-mismatch']]) if (Object.hasOwn(expected, key) && (!normalize || observed !== normalize)) blockers.push(blocker);
  return Object.freeze({ valid: blockers.length === 0, identity: Object.freeze({ repository, prNumber, branch, sourceHead, sourceTree, baseSha, workflowRunId, workflowRunAttempt, mergeStateStatus, reviewAdjudication: mergeStateStatus === 'UNSTABLE' ? 'clean-independent-review' : 'native-clean' }), blockers: Object.freeze(unique(blockers)), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_EVIDENCE_BLOCKED' : 'PERSONAL_REPOSITORY_EVIDENCE_READY' });
}

function rulesOfType(activeRules, type) { return activeRules.filter((rule) => text(rule?.type).toLowerCase() === type); }
function configurationNotProved(blockers, detail) { blockers.push(`CONFIGURATION_NOT_PROVED:${detail}`); }
...
export function validatePersonalRepositoryConfiguration(input = {}, options = {}) {
  ...
  const environment = validateProtectedEnvironment(input.environment, { expectedName: OPERATOR_MERGE_ENVIRONMENT, expectedReviewer: OPERATOR_MERGE_REVIEWER });
  ...
  if (repository?.private !== false || text(repository?.visibility).toLowerCase() !== 'public') blockers.push('personal-repository-rules-api-not-public');
  ...
  const pullRequestRules = rulesOfType(rules, 'pull_request'), statusCheckRules = rulesOfType(rules, 'required_status_checks'), mergeQueueRules = rulesOfType(rules, 'merge_queue'), nonFastForwardRules = rulesOfType(rules, 'non_fast_forward'), deletionRules = rulesOfType(rules, 'deletion');
  ...
  const requiredStatusChecks = Array.isArray(statusParameters.required_status_checks) ? statusParameters.required_status_checks : null;
  if (!requiredStatusChecks) configurationNotProved(blockers, 'personal-repository-required-status-check-list');
  else if (requiredStatusChecks.filter((check) => text(check?.context) === requiredCheck && strictPositiveInteger(check?.integration_id) === expectedIntegrationId).length !== 1) blockers.push('personal-repository-required-check-not-exact');
  if (statusParameters.strict_required_status_checks_policy !== true) blockers.push('personal-repository-strict-status-policy-not-enforced');
  const activeRulesetIds = unique(rules.map((rule) => strictPositiveInteger(rule?.ruleset_id)).filter(Boolean));
  if (!rules.length || activeRulesetIds.length === 0 || rules.some((rule) => !strictPositiveInteger(rule?.ruleset_id))) configurationNotProved(blockers, 'personal-repository-active-rule-identities');
  const suppliedRulesets = rulesets || [];
  const suppliedRulesetIds = suppliedRulesets.map((ruleset) => strictPositiveInteger(ruleset?.id));
  if (suppliedRulesetIds.some((id) => !id) || suppliedRulesetIds.length !== activeRulesetIds.length || activeRulesetIds.some((id) => !suppliedRulesetIds.includes(id))) configurationNotProved(blockers, 'personal-repository-ruleset-evidence-not-exact');
  for (const ruleset of suppliedRulesets) {
    const rulesetId = strictPositiveInteger(ruleset?.id);
    if (text(ruleset?.enforcement).toLowerCase() !== 'active') blockers.push(`personal-repository-ruleset-not-active:${rulesetId || 'unknown'}`);
    if (!EXPLICIT_TIMEZONE.test(text(ruleset?.updated_at)) || !Number.isFinite(Date.parse(ruleset.updated_at))) configurationNotProved(blockers, `personal-repository-ruleset-updated-at:${rulesetId || 'unknown'}`);
    if (requireBypassProof && !Array.isArray(ruleset?.bypass_actors)) configurationNotProved(blockers, `personal-repository-ruleset-bypass-actors:${rulesetId || 'unknown'}`); else if (Array.isArray(ruleset?.bypass_actors) && ruleset.bypass_actors.length !== 0) blockers.push(`personal-repository-ruleset-bypass-present:${rulesetId || 'unknown'}`);
  }
  return Object.freeze({ valid: blockers.length === 0, environment, requiredCheck, requiredCheckIntegrationId: expectedIntegrationId, activeRulesetIds: Object.freeze(activeRulesetIds), bypassProven: requireBypassProof, blockers: Object.freeze(unique(blockers)), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_CONFIGURATION_BLOCKED' : requireBypassProof ? 'PERSONAL_REPOSITORY_CONFIGURATION_READY' : 'PERSONAL_REPOSITORY_CONFIGURATION_PARTIAL_PROOF_READY' });
}

const APPROVAL_RECEIPT_KEYS = Object.freeze(['schemaVersion','kind','repository','prNumber','branch','sourceHead','sourceTree','baseSha','workflowPath','workflowRunId','workflowRunAttempt','environment','requiredReviewer','independentReviewWorkflowRunId','independentReviewWorkflowRunAttempt','independentReviewArtifactId','independentReviewArtifactDigest','independentReviewPayloadSha256','evidenceSha256','approvedAtUtc','authority','mergeMethod','reusableAcrossHeads','reusableAcrossBases']);

export function buildPersonalRepositoryApprovalReceipt(input = {}) {
  if (input?.evidence?.finalVerdict !== 'PERSONAL_REPOSITORY_EVIDENCE_READY' || input?.configuration?.finalVerdict !== 'PERSONAL_REPOSITORY_CONFIGURATION_READY' || input?.workflows?.finalVerdict !== 'PERSONAL_REPOSITORY_WORKFLOWS_READY') throw new Error('Personal-repository approval requires ready identity, workflow and configuration evidence.');
  const identity = input.evidence.identity;
  const receipt = { schemaVersion: 'stephanos.personal-repository-approval.v1', kind: 'stephanos.personal-repository.protected-squash-approval', repository: identity.repository, prNumber: identity.prNumber, branch: identity.branch, sourceHead: identity.sourceHead, sourceTree: identity.sourceTree, baseSha: identity.baseSha, workflowPath: PERSONAL_REPOSITORY_WORKFLOW_PATH, workflowRunId: identity.workflowRunId, workflowRunAttempt: identity.workflowRunAttempt, environment: OPERATOR_MERGE_ENVIRONMENT, requiredReviewer: OPERATOR_MERGE_REVIEWER, independentReviewWorkflowRunId: strictPositiveInteger(input.independentReviewWorkflowRunId), independentReviewWorkflowRunAttempt: strictPositiveInteger(input.independentReviewWorkflowRunAttempt), independentReviewArtifactId: strictPositiveInteger(input.independentReviewArtifactId), independentReviewArtifactDigest: text(input.independentReviewArtifactDigest).toLowerCase(), independentReviewPayloadSha256: text(input.independentReviewPayloadSha256).toLowerCase(), evidenceSha256: text(input.evidenceSha256).toLowerCase(), approvedAtUtc: text(input.approvedAtUtc), authority: PERSONAL_REPOSITORY_AUTHORITY, mergeMethod: 'squash', reusableAcrossHeads: false, reusableAcrossBases: false };
...
export function validatePersonalRepositoryApprovalReceipt(receipt = {}, expected = {}) {
  ...
  for (const [key, blocker] of [['sourceHead','personal-repository-approval-head-invalid'],['sourceTree','personal-repository-approval-tree-invalid'],['baseSha','personal-repository-approval-base-invalid']]) if (!SHA_PATTERN.test(text(receipt[key]).toLowerCase())) blockers.push(blocker);
  ...
  if (!EXPLICIT_TIMEZONE.test(text(receipt.approvedAtUtc)) || !Number.isFinite(Date.parse(receipt.approvedAtUtc))) blockers.push('personal-repository-approval-time-invalid');
  ...
  for (const [key, blocker] of [['repository','personal-repository-approval-repository-mismatch'],['prNumber','personal-repository-approval-pr-mismatch'],['branch','personal-repository-approval-branch-mismatch'],['sourceHead','personal-repository-approval-head-mismatch'],['sourceTree','personal-repository-approval-tree-mismatch'],['baseSha','personal-repository-approval-base-mismatch'],['workflowRunId','personal-repository-approval-run-mismatch'],['workflowRunAttempt','personal-repository-approval-attempt-mismatch'],['independentReviewWorkflowRunId','personal-repository-approval-review-run-mismatch'],['independentReviewWorkflowRunAttempt','personal-repository-approval-review-attempt-mismatch'],['independentReviewArtifactId','personal-repository-approval-artifact-id-mismatch'],['independentReviewArtifactDigest','personal-repository-approval-artifact-digest-mismatch'],['independentReviewPayloadSha256','personal-repository-approval-payload-digest-mismatch'],['evidenceSha256','personal-repository-approval-evidence-digest-mismatch']]) if (Object.hasOwn(expected, key) && receipt[key] !== expected[key]) blockers.push(blocker);
  return Object.freeze({ valid: blockers.length === 0, blockers: Object.freeze(unique(blockers)), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_APPROVAL_BLOCKED' : 'PERSONAL_REPOSITORY_APPROVAL_READY' });
}
...
export function validatePersonalRepositorySquashCompletion(input = {}, expected = {}) {
  ...
  const expectedHead = text(expected.sourceHead).toLowerCase(), expectedTree = text(expected.sourceTree).toLowerCase(), expectedBase = text(expected.baseSha).toLowerCase();
  if (input?.mergeResponse?.merged !== true || !SHA_PATTERN.test(mergeSha)) blockers.push('personal-repository-merge-response-invalid');
  if (input?.pullRequest?.merged !== true || text(input?.pullRequest?.merge_commit_sha).toLowerCase() !== mergeSha) blockers.push('personal-repository-pr-not-exactly-merged');
  ...
  if (parents.length !== 1 || text(parents[0]?.sha ?? parents[0]).toLowerCase() !== expectedBase) blockers.push('personal-repository-squash-parent-not-exact-base');
  ...
  return Object.freeze({ valid: blockers.length === 0, mergeSha, mainSha, treeSha: commitTree, branchSha, blockers: Object.freeze(unique(blockers)), finalVerdict: blockers.length ? 'PERSONAL_REPOSITORY_SQUASH_COMPLETION_BLOCKED' : 'PERSONAL_REPOSITORY_SQUASH_COMPLETION_READY' });
}
