...
} from '../../scripts/battle-bridge-goal-discovery-heartbeat.mjs';
...
test('Lane 7 receives the canonical GitHub CLI identity from goal discovery', async () => {
  const observed = [];
...
test('Lane 7 source pins the Windows GitHub CLI path instead of hidden-task PATH lookup', async () => {
  const source = await readFile(new URL('../../scripts/battle-bridge-goal-discovery-heartbeat.mjs', import.meta.url), 'utf8');
