...
function isOpen(pr) {
  return !['closed', 'merged'].includes(text(pr.state || pr.status).toLowerCase());
}
...
function prStatus(pr) {
  if (hasHeadDrift(pr)) return { status: BLOCKED, reason: 'claimed-head drift blocked' };
  ...
  return { status: isOpen(pr) ? 'open' : 'closed', reason: text(pr.status || pr.state || 'open') };
}
