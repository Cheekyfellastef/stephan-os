...
const REVIEWED_LINEAGE_ANCHOR = 'd3fff6197827564490d94734d2c92381297db300';
...
const REVIEWED_SOURCE_MANIFEST = Object.freeze({
  ...
  'scripts/windows/restart-approved-stephanos-runtime.ps1': Object.freeze({
    blobSha: '6d25bd478dfef5a198e7792005ecdc6eb0b3233c',
    size: 56189,
  }),
...
function reviewRestart(source, path, findings) {
  ...
  forbidPattern(findings, executableSource, /\$heartbeatTimestampUtc -gt \$observedAtUtc\.AddSeconds\(|Stop-Process\s+-Id\s+\$oldWorker(?:Recheck)?\.ProcessId/i, 'watchdog-restart-existing-worker-authority-widened', 'Existing-worker cleanup must reject every future heartbeat and must not re-resolve a PID for termination.', path);
