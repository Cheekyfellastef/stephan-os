...
const WSL2_BLOB_SHA = '5a74e3f52521118ff42b5ee864566509127fd8a1';
...
function inspectMailboxRecoveryRequest(source) {
  ...
  requirePattern(findings, source, /Start-ScheduledTask\s+-TaskName\s+\$taskName/i, 'mailbox-rollover-recovery-start-not-fixed', 'Recovery request may wake only the fixed recovery mesh task.', path);
  forbidPattern(findings, source, /Invoke-Expression|\biex\b|\bStart-Process\b|\bStop-Process\b|\btaskkill(?:\.exe)?\b|cmd(?:\.exe)?\s+\/c|powershell(?:\.exe)?\s+-command/i, 'mailbox-rollover-recovery-generic-execution-forbidden', 'Recovery request must not gain arbitrary process or shell execution authority.', path);
  forbidPattern(findings, source, /(?:^|[\r\n;{}|&]\s*)(?:git(?:\.exe)?|gh(?:\.exe)?)(?=\s|$)/im, 'mailbox-rollover-recovery-source-control-forbidden', 'Recovery request must not mutate or directly operate source control.', path);
  return findings;
}
...
function analyzeMailboxRolloverReview(input = {}) {
  ...
  if (repository !== REPOSITORY || !SHA40.test(sourceHead) || !SHA40.test(baseSha) || !exactMailboxRolloverEscalation(input.analysis) || !exactMailboxRolloverLineage(input.lineageEvidence, sourceHead, baseSha)) {
    return Object.freeze({ eligible: false, clean: false, findings: Object.freeze([]), reviewedPaths: Object.freeze([]), proofRefs: Object.freeze([]), finalVerdict: 'WINDOWS_AUTHORITY_MAILBOX_ROLLOVER_SPECIALIST_NOT_ELIGIBLE' });
  }
  const sources = Array.isArray(input.sources) ? input.sources : [];
  const findings = [];
  const proofRefs = [];
  for (const path of WINDOWS_AUTHORITY_MAILBOX_ROLLOVER_PATHS_V1) {
    const matches = sources.filter((source) => source?.path === path);
    if (matches.length !== 1 || !exactMailboxRolloverSource(matches[0], path, sourceHead)) {
      findings.push(mailboxRolloverFinding('mailbox-rollover-source-evidence-invalid', 'Exactly one immutable exact-head source record is required for each reviewed mailbox rollover path.', path));
      continue;
    }
    const localFindings = path.endsWith('install-battle-bridge-github-command-mailbox.ps1')
      ? inspectMailboxInstaller(matches[0].content)
      : inspectMailboxRecoveryRequest(matches[0].content);
    findings.push(...localFindings);
    proofRefs.push(`proofs/windows-authority-mailbox-rollover/${path}@${sourceHead}#${matches[0].blobSha}:${matches[0].size}`);
  }
  if (sources.length !== WINDOWS_AUTHORITY_MAILBOX_ROLLOVER_PATHS_V1.length) {
    findings.push(mailboxRolloverFinding('mailbox-rollover-source-estate-widened', 'Specialist source estate must contain exactly the two reviewed mailbox rollover files.', WINDOWS_AUTHORITY_MAILBOX_ROLLOVER_PATHS_V1[0]));
  }
  ...
  return Object.freeze({
    ...
    finalVerdict: clean ? 'WINDOWS_AUTHORITY_MAILBOX_ROLLOVER_SPECIALIST_CLEAN' : 'WINDOWS_AUTHORITY_MAILBOX_ROLLOVER_SPECIALIST_FINDINGS',
  });
}

const baseModule = provePinnedModule(BASE_PATH, BASE_BLOB_SHA);
proveLegacyRoutingInvariants(baseModule.content);
const core = await import(baseModule.url.href);
...
const ignitionConvergenceModule = provePinnedModule(IGNITION_CONVERGENCE_PATH, IGNITION_CONVERGENCE_BLOB_SHA);
const ignitionConvergence = await import(ignitionConvergenceModule.url.href);
const missionWorkerCleanupModule = provePinnedModule(MISSION_WORKER_CLEANUP_PATH, MISSION_WORKER_CLEANUP_BLOB_SHA);
const missionWorkerCleanup = await import(missionWorkerCleanupModule.url.href);
...
export function analyzeWindowsAuthoritySpecialistReview(input = {}) {
  ...
  return core.analyzeWindowsAuthoritySpecialistReview(input);
}
