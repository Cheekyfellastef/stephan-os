...
test('valid diagnostic link is sanitized and remains a serialized CONTROL operation', () => {
  ...
  assert.equal(result.command.targetRequestId, '');
  assert.equal(
...
test('batch selection restores diagnostic operation after legacy envelope validation', () => {
  const body = `\`\`\`${mailbox.BATTLE_BRIDGE_GITHUB_COMMAND_MARKER}\n${JSON.stringify(command())}\n\`\`\``;
  const result = mailbox.selectBattleBridgeGitHubCommandBatch([{
    id: 1001,
    html_url: 'https://github.com/Cheekyfellastef/stephan-os/issues/1507#issuecomment-1001',
    body,
