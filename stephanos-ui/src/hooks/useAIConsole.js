...
function buildBuilderWorkbenchExecutionMetadata(builderWorkbenchProjection = {}, {
  ...
  return {
    ...
    openclaw_route_id: workbench?.openClawSanityGate?.routeId || 'unknown',
    openclaw_route_trust_status: workbench?.openClawSanityGate?.routeTrustStatus || 'untrusted',
    openclaw_route_task_frame_status: workbench?.openClawSanityGate?.routeTaskFrameStatus || 'unknown',
    openclaw_route_session_id: workbench?.openClawSanityGate?.routeSessionId || 'unknown',
    openclaw_active_session_count: workbench?.openClawSanityGate?.activeSessionCount || '0',
    openclaw_active_session_contamination_risk: workbench?.openClawSanityGate?.activeSessionContaminationRisk || 'no',
    openclaw_route_model_pinned: workbench?.openClawSanityGate?.routeModelPinned || 'unknown',
    openclaw_route_model_configured_primary: workbench?.openClawSanityGate?.routeModelConfiguredPrimary || 'unknown',
    openclaw_route_model_mismatch_detected: workbench?.openClawSanityGate?.routeModelMismatchDetected || 'no',
    openclaw_model_pin_mismatch_warnings: (workbench?.openClawSanityGate?.modelPinMismatchWarnings || []).join(' | ') || 'none',
    openclaw_plaintext_token_security_warning: workbench?.openClawSanityGate?.plaintextTokenSecurityWarning || 'no',
    openclaw_doctor_non_blocking_findings: (workbench?.openClawSanityGate?.doctorNonBlockingFindings || []).join(' | ') || 'none',
    openclaw_dashboard_failure_examples: (workbench?.openClawSanityGate?.dashboardFailureExamples || []).join(' | ') || 'none',
    openclaw_minimum_viable_route_recommendation: workbench?.openClawSanityGate?.minimumViableRouteRecommendation || 'Use stephanos-scout / llama3.2 CLI for bounded source-pack processing only; OpenClaw cannot mutate files.',
    openclaw_sanity_status: workbench?.openClawSanityGate?.sanityStatus || 'idle',
    openclaw_sanity_failure_reason: workbench?.openClawSanityGate?.failureReason || 'none',
    openclaw_exact_response_status: workbench?.openClawSanityGate?.exactResponseStatus || 'unknown',
    openclaw_exact_response_payload: workbench?.openClawSanityGate?.exactResponsePayload || 'none',
    openclaw_cli_banner_ignored: workbench?.openClawSanityGate?.cliBannerIgnored || 'no',
    openclaw_template_leakage_detected: workbench?.openClawSanityGate?.templateLeakageDetected || 'no',
    openclaw_wrong_repo_path_detected: workbench?.openClawSanityGate?.wrongRepoPathDetected || 'no',
    openclaw_trusted_for_builder_routing: workbench?.openClawSanityGate?.trustedForBuilderRouting || 'no',
    openclaw_sanity_next_operator_action: workbench?.openClawSanityGate?.nextOperatorAction || 'Paste an OpenClaw result to run the sanity gate before Builder Mesh routing.',
    openclaw_patch_planner_status: workbench?.openClawPatchPlanner?.patchPlannerStatus || 'idle',
    openclaw_patch_planner_risk_level: workbench?.openClawPatchPlanner?.riskLevel || 'unknown',
    openclaw_patch_planner_likely_file_count: String(workbench?.openClawPatchPlanner?.likelyFiles?.length ?? 0),
    openclaw_patch_planner_required_test_count: String(workbench?.openClawPatchPlanner?.requiredTests?.length ?? 0),
    openclaw_patch_planner_browser_proof_required: workbench?.openClawPatchPlanner?.browserProofRequired || 'unknown',
    openclaw_patch_planner_forbidden_action_detected: workbench?.openClawPatchPlanner?.forbiddenActionsDetected || 'no',
    openclaw_patch_planner_placeholder_leakage_detected: workbench?.openClawPatchPlanner?.placeholderLeakageDetected || 'no',
    ...
    openclaw_source_pack_judgment_stale: workbench?.openClawSourcePackRunner?.sourcePackJudgmentStale || 'no',
    openclaw_source_pack_last_judged_text_length: String(workbench?.openClawSourcePackRunner?.sourcePackLastJudgedTextLength ?? 0),
    openclaw_source_pack_current_text_length: String(workbench?.openClawSourcePackRunner?.sourcePackCurrentTextLength ?? 0),
    openclaw_source_pack_last_judged_output_length: String(workbench?.openClawSourcePackRunner?.sourcePackLastJudgedOutputLength ?? 0),
    openclaw_source_pack_current_output_length: String(workbench?.openClawSourcePackRunner?.sourcePackCurrentOutputLength ?? 0),
    openclaw_source_pack_projection_written: workbench?.openClawSourcePackRunner?.sourcePackProjectionWritten || 'no',
    openclaw_source_pack_projection_source: workbench?.openClawSourcePackRunner?.sourcePackProjectionSource || 'source-pack-runner-idle',
...
function formatBuilderMeshAnswer(projection = {}, projectAwareness = {}, prompt = '') {
  ...
  return [
    ...
    `${openClawSanityGate.sanityStatus === 'failed' || openClawSourcePackRunner.sourcePackStatus === 'failed' ? 'OpenClaw is blocked from Builder Mesh research/patch-planning routing until sanity and source-pack intake are restored; build routing remains blocked without operator approval.' : 'OpenClaw can help only as a bounded source-pack processor through the llama3.2 CLI route unless separate route proof exists.'} OpenClaw cannot browse on its own or mutate files/build, dashboard/qwen remain untrusted, Codex remains fallback implementation capacity only, and operator approval is required before mutation or canon/build promotion.`,
...
export function useAIConsole() {
  ...
  async function submitPrompt(rawPrompt, { telemetryEntries = [], orchestrationTruth = null, submissionSource = 'stephanos-mission-console', submissionRoute = 'assistant-router' } = {}) {
    ...
    const commandDeckIntakeMetadata = {
      ...
      browser_proof_intake_status: commandDeckUniversalIntake.evidenceReturnIntakeProjection?.browserProofIntakeStatus || 'unavailable',
      browser_proof_known_caveat_present: commandDeckUniversalIntake.evidenceReturnIntakeProjection?.browserProofKnownCaveatPresent ? 'yes' : 'no',
      browser_proof_caveat_blocking: commandDeckUniversalIntake.evidenceReturnIntakeProjection?.browserProofCaveatBlocking ? 'yes' : 'no',
      browser_proof_rejection_reason: commandDeckUniversalIntake.evidenceReturnIntakeProjection?.browserProofRejectionReason || 'none',
      browser_proof_accepted_with_caveat: commandDeckUniversalIntake.evidenceReturnIntakeProjection?.browserProofAcceptedWithCaveat ? 'yes' : 'no',
