...
export function attachPrEvidenceToEnvelope(envelope, prEvidence = null) {
  return {
    ...
    prEvidence: {
      ...
      fetchDisabled: asText(prEvidence?.fetchDiagnostics?.github_pr_evidence_fetch_disabled, 'yes'),
      fetchDisabledReason: asText(prEvidence?.fetchDiagnostics?.github_pr_evidence_fetch_disabled_reason, 'live-fetch-disabled-by-default'),
...
export function projectEnvelopeToExecutionMetadata(envelope = {}) {
  ...
  return {
    ...
    github_pr_evidence_fetch_disabled: asText(envelope?.prEvidence?.fetchDisabled, 'yes'),
    github_pr_evidence_fetch_disabled_reason: asText(envelope?.prEvidence?.fetchDisabledReason, 'live-fetch-disabled-by-default'),
