...
test('fails honestly when canonical VR projection is missing', () => {
  ...
  assert.deepEqual(vrResearchContextProvider.getNextAction({ now: NOW }), ['Refresh and validate the canonical vr-research Shared Workspace projection before answering from it.']);
});

test('projects fresh canonical VR truth into known observed inferred proposed and blocked sections', () => {
  const summary = buildVrResearchContextSummary({ now: NOW, sharedWorkspace: { domains: { 'vr-research': projection() } } });
...
test('marks stale VR truth as non-ready and requests refresh', () => {
  const staleProjection = projection({ updatedAt: '2026-07-30T00:00:00Z', staleAfterMs: 60 * 60 * 1000 });
  const inspection = inspectVrResearchProjection({ now: NOW, vrResearchProjection: staleProjection });
  assert.equal(inspection.status, 'STALE'); assert.equal(inspection.proofState, 'stale'); assert.match(inspection.warning, /stale/i);
});

test('rejects a conflicting projection identity', () => {
  const inspection = inspectVrResearchProjection({ now: NOW, vrResearchProjection: projection({ domainId: 'private-vr-agent-memory' }) });
  assert.equal(inspection.status, 'INVALID'); assert.equal(inspection.proofState, 'invalid'); assert.match(inspection.warning, /identity is invalid/i);
});
