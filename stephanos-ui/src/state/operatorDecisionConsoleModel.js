...
export function buildOperatorDecisionQueue(input = {}) {
  ...
  if (codexPrRepairContract.repairCompleteness && codexPrRepairContract.repairCompleteness !== 'repair_complete') {
    decisions.push(createDecision({ missionId, createdAt: now, decisionType:'review_codex_pr_repair_blocker', title:'Review Codex PR repair blocker', summary:'Codex PR repair contract is incomplete.', sourceSystem:'codex_pr_repair_contract', sourceStatus:codexPrRepairContract.repairCompleteness, riskLevel:'high', urgency:'high', recommendedAction:'review_codex_pr_repair_blocker', allowedActions:['request-codex-repair-contract-update','hold-for-operator'], blockedActions:['merge-red-pr','assume-local-tests-enough','accept-dist-only-fix-for-source-failure'], requiresExplicitApproval:true, authorityImplication:'pr_repair_incomplete', relatedSubsystems:['task_finisher','verification_judge','pr_evidence'], status:'blocked', warningLevel:'high', reason:asText(codexPrRepairContract.nextAction, 'Codex PR repair requires operator review.') }));
  }
  if (codexPrRepairContract.repairCompleteness === 'blocked_codex_cannot_push') {
    decisions.push(createDecision({ missionId, createdAt: now, decisionType:'choose_repair_path', title:'Choose PR repair path', summary:'Codex cannot push to target PR branch.', sourceSystem:'codex_pr_repair_contract', sourceStatus:'blocked_codex_cannot_push', riskLevel:'high', urgency:'high', recommendedAction:'choose_repair_path', allowedActions:['ask Codex to recreate task with GitHub access','close PR and regenerate from main','retry in connected environment','manual emergency intervention'], blockedActions:['merge-red-pr','assume-local-tests-enough','accept-dist-only-fix-for-source-failure'], requiresExplicitApproval:true, authorityImplication:'operator_choice_required', relatedSubsystems:['operator_console'], status:'blocked', warningLevel:'high', reason:'Codex must repair target branch or report blocked state.' }));
  }
