...
export function buildSupportSnapshot({
  ...
  const lines = [
    ...
    `Fast Response Lane Active: ${asText(runtimeStatus?.lastFastResponseLaneActive, 'false')}`,
    `Fast Response Lane Reason: ${asText(runtimeStatus?.lastFastResponseLaneReason)}`,
    `Fast Response Model: ${asText(runtimeStatus?.lastFastResponseModel)}`,
    `Escalation Model: ${asText(runtimeStatus?.lastEscalationModel)}`,
    `Escalation Reason: ${asText(runtimeStatus?.lastEscalationReason)}`,
