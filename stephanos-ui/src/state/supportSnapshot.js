...
function isDefaultWorkbenchMetadataValue(key = '', value = '') {
  ...
  if (key.startsWith('openclaw_source_pack_')) return ['none', 'unknown', 'n/a', 'idle', 'no', '0', 'not-attempted', 'copy the source pack cli prompt and paste a bounded source-pack result.'].includes(normalized);
...
function buildWorkbenchMetadataFromProjection(workbench = {}, source = 'none') {
  ...
  return {
    ...
    openclaw_source_pack_intake_button_clicked: projection.openClawSourcePackRunner?.diagnostics?.intakeButtonClicked || projection.openClawSourcePackDiagnostics?.intakeButtonClicked || 'no',
    openclaw_source_pack_text_length: projection.openClawSourcePackRunner?.diagnostics?.sourcePackTextLength || projection.openClawSourcePackDiagnostics?.sourcePackTextLength || '0',
    openclaw_source_pack_output_length: projection.openClawSourcePackRunner?.diagnostics?.sourcePackOutputLength || projection.openClawSourcePackDiagnostics?.sourcePackOutputLength || '0',
    openclaw_source_pack_judgment_attempted: projection.openClawSourcePackRunner?.diagnostics?.judgmentAttempted || projection.openClawSourcePackDiagnostics?.judgmentAttempted || 'no',
    openclaw_source_pack_judgment_result_status: projection.openClawSourcePackRunner?.diagnostics?.judgmentResultStatus || projection.openClawSourcePackDiagnostics?.judgmentResultStatus || 'not-attempted',
    openclaw_source_pack_projection_written: projection.openClawSourcePackRunner?.diagnostics?.projectionWritten || projection.openClawSourcePackDiagnostics?.projectionWritten || 'no',
    openclaw_source_pack_projection_write_boundary: projection.openClawSourcePackRunner?.diagnostics?.projectionWriteBoundary || projection.openClawSourcePackDiagnostics?.projectionWriteBoundary || 'none',
    openclaw_source_pack_snapshot_source: projection.openClawSourcePackRunner?.diagnostics?.snapshotSource || projection.openClawSourcePackDiagnostics?.snapshotSource || source,
...
function resolveBuilderWorkbenchSupportMetadata(executionMetadata = {}, runtimeStatus = {}) {
  ...
  const fields = [
    ...
    ['openclaw_source_pack_intake_button_clicked', 'no'],
    ['openclaw_source_pack_text_length', '0'],
    ['openclaw_source_pack_output_length', '0'],
    ['openclaw_source_pack_judgment_attempted', 'no'],
    ['openclaw_source_pack_judgment_result_status', 'not-attempted'],
    ['openclaw_source_pack_projection_written', 'no'],
    ['openclaw_source_pack_projection_write_boundary', 'none'],
    ['openclaw_source_pack_snapshot_source', 'none'],
...
export function buildSupportSnapshot({
  ...
  const lines = [
    ...
    `Mission Evidence Completeness: ${asText(runtimeStatus?.missionEvidenceCompleteness, 'low')}`,
    `Mission Evidence Latest Event: ${asText(runtimeStatus?.missionEvidenceLatestEvent, 'none')}`,
    `Mission Evidence Next Required: ${asText(runtimeStatus?.missionEvidenceNextRequired, 'none')}`,
    ...
    `Source Pack Intake Button Clicked: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_intake_button_clicked, 'no')}`,
    `Source Pack Text Length: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_text_length, '0')}`,
    `Source Pack Output Length: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_output_length, '0')}`,
    `Source Pack Judgment Attempted: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_judgment_attempted, 'no')}`,
    `Source Pack Judgment Result Status: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_judgment_result_status, 'not-attempted')}`,
    `Source Pack Projection Written: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_projection_written, 'no')}`,
    `Source Pack Projection Write Boundary: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_projection_write_boundary, 'none')}`,
    `Source Pack Snapshot Source: ${asText(builderWorkbenchSupportMetadata.openclaw_source_pack_snapshot_source, 'none')}`,
