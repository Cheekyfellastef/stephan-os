...
test('uiLayout forces missionConsolePanel open when persisted pane order omitted it', () => {
  assert.match(source, /shouldForceMissionConsoleOpen = !persistedPaneOrderIncludesMissionConsole\(persistedSession\)/m);
  assert.match(source, /\{ \.\.\.effectiveUiLayoutBase, missionConsolePanel: true \}/m);
});
