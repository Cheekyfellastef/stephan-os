...
export function buildCodexDispatchPacket(input = {}) {
  const bridgeDraft = input.missionRepairCodexBridge?.codexDispatchPacketDraft || {};
  ...
  const targetSubsystems = asList(input.targetSubsystems?.length ? input.targetSubsystems : (bridgeDraft.targetSubsystems?.length ? bridgeDraft.targetSubsystems : input.chatContextPack?.affectedSubsystems));
  const canonRules = asList(input.chatContextPack?.relevantCanon?.map((rule) => rule.text));
  const requiredTests = asList(input.requiredTests?.length ? input.requiredTests : (bridgeDraft.requiredTests?.length ? bridgeDraft.requiredTests : ['node --test stephanos-ui/src/state/chatContextOrchestrator.test.mjs', 'node --test stephanos-ui/src/state/responsePlanner.test.mjs', 'npm run stephanos:verify']));
  ...
  const forbiddenActions = asList(input.forbiddenActions?.length ? input.forbiddenActions : (bridgeDraft.forbiddenActions?.length ? bridgeDraft.forbiddenActions : [
  ...
  const missionTitle = asText(input.missionTitle || bridgeDraft.missionTitle, 'Codex build/repair mission draft');
  return {
    version: VERSION,
    status,
    ...
    operatorIntent,
    missionTitle,
    missionClass: asText(input.missionClass || bridgeDraft.missionClass, operatorIntent === 'codex-prompt' ? 'prompt-only' : 'dispatch-prep'),
    targetSubsystems,
