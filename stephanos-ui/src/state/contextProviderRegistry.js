...
const builtinProviders = [
  ...
  },
  {
    ...
    getSummary: (input) => ({
      ...
      rehydrated: String(input.operatorProfile?.rehydrated ? 'yes' : 'no'),
      storageKey: String(input.operatorProfile?.storageKey || 'stephanos.operator.profile.v1'),
      storageReadStatus: String(input.operatorProfile?.storageReadStatus || 'missing'),
      lastReadAt: String(input.operatorProfile?.lastReadAt || 'unknown'),
      lastWriteAt: String(input.operatorProfile?.lastWriteAt || 'unknown'),
  ...
  },
  {
    ...
    getSummary: (input) => ({
      ...
      summary: String(input.chatContinuity?.summaries?.[0]?.summary || input.chatContinuity?.recentTopic || 'none'),
      activeMission: String(input.chatContinuity?.activeMission || 'unknown'),
      recentDecisions: normalizeList(input.chatContinuity?.recentDecisions),
      openQuestions: normalizeList(input.chatContinuity?.openQuestions),
      nextAction: String(input.chatContinuity?.nextAction || 'unknown'),
      warnings: normalizeList(input.chatContinuity?.warnings),
      seededFromExistingHistory: String(input.chatContinuity?.seededFromExistingHistory ? 'yes' : 'no'),
      continuitySource: String(input.chatContinuity?.continuitySource || 'none'),
      summaryCount: Number(input.chatContinuity?.summaries?.length || 0),
      rawTranscriptStored: 'no',
      sourceRefs: ['chatContinuity'],
    }),
    getWarnings: (input) => normalizeList(input.chatContinuity?.warnings),
    getNextAction: (input) => normalizeList([input.chatContinuity?.nextAction]),
    getProofState: () => 'ready',
    getCanonLinks: () => ['canon.source_owns_behavior'],
    getSourceRefs: () => ['chatContinuity'],
  },
  {
    id: 'agentState', label: 'Agent State', priority: 80,
    getSummary: (input) => ({
      selectedAgent: String(input.agentState?.selectedAgentId || 'unknown'),
      actingAgent: String(input.agentState?.actingAgentId || 'unknown'),
      activeAgentIds: normalizeList(input.agentState?.activeAgentIds),
      waitingAgentIds: normalizeList(input.agentState?.waitingAgentIds),
      blockedAgentIds: normalizeList(input.agentState?.blockedAgentIds),
      assignmentCounts: Number(input.agentState?.assignments?.length || input.agentState?.assignmentCount || 0),
      approvalRequired: String(input.agentState?.approvalRequired || 'unknown'),
      codexAssigned: String(input.agentState?.codexAssigned || 'no'),
      openClawAssigned: String(input.agentState?.openClawAssigned || 'no'),
      blockers: normalizeList(input.agentState?.blockers),
      recommendedAgents: normalizeList(input.agentState?.recommendedAgents),
      advisoryNote: String(input.agentState?.advisoryNote || 'none'),
    }),
    getWarnings: (input) => normalizeList(input.agentState?.blockers),
    getNextAction: (input) => normalizeList([input.agentState?.nextAction]),
    getProofState: () => 'ready',
