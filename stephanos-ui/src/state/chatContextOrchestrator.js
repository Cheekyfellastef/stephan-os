...
export const INTENT_RULES = [
  ...
  },
  {
    ...
    pattern: /\b(who should (work on|handle|do) this|who should do (the )?next task|what builder should (take|handle|do)|is openclaw online|start openclaw bridge|show openclaw proof command|bring openclaw online( inside stephanos)?|openclaw control bridge|can openclaw plan[^.!?]{0,80}(patch|next patch)|openclaw patch planner|why is ignition blocked after using openclaw|openclaw hard-block files|openclaw workspace (hygiene|dirt)|what should i do with (the )?openclaw hard-block files|can openclaw help|openclaw[^.!?]{0,80}(continue|carry on|keep going)[^.!?]{0,80}(build|stephanos)|continue[^.!?]{0,80}(stephanos )?build[^.!?]{0,80}without[^.!?]{0,40}codex|can my local ais? do this|can local ais? help|how do we avoid using codex|without codex|without using metered codex|without hitting the meter|keep building without (using )?(metered )?codex|zero-cost builder|builder mesh|builder workbench|workbench status|local ai.*openclaw|codex meter)\b/,
  },
  {
    id: 'work-routing',
    responseMode: 'work-routing',
...
export function buildChatContextPack(input = {}) {
  ...
  else if (missionPlanningTask) inferredSubsystems = ['builder-mesh', 'builder-workbench', 'openclaw-control', 'mission-console', 'proof'];
