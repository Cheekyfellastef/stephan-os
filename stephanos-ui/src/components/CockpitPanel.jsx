...
export function resolveOperatorProofConciergeRenderState(operatorProofConcierge = {}) {
  ...
  const payloadFirstLine = (primaryProofCopyPacket?.payload || primaryProofCopyPacket?.packetText || '').split(/\r?\n/).map((line) => line.trim()).find(Boolean) || 'none';
  ...
  return {
    ...
    primarySource,
    payloadFirstLine,
    proofStateContradictionDetected: operatorProofConcierge.proofStateContradictionDetected || 'no',
    sourceProjectionKey: 'operatorProofConcierge.copyPacket',
    proofSignature: operatorProofConcierge.proofSignature || operatorProofConcierge.canonicalProofSignature || 'unavailable',
...
export default function CockpitPanel({ forceOpen = false, standalone = false, telemetryEntries = [], finalAgentView = null, cockpitProjectionOverride = null } = {}) {
  ...
  const handleCopyConciergePacket = useCallback(async () => {
    ...
    const primaryProofCopyPacket = concierge.copyPacket;
    await copyConciergePacket({ packet: primaryProofCopyPacket, setCopyState: setConciergeCopyState, buttonRole: 'primary-proof-copy', buttonTestId: 'operator-proof-concierge-primary-copy' });
  ...
  const {
    ...
    payloadFirstLine: proofConciergeRenderPayloadFirstLine,
    proofStateContradictionDetected: proofConciergeRenderContradictionDetected,
    sourceProjectionKey: proofConciergeRenderSourceProjectionKey,
    proofSignature: proofConciergeRenderProofSignature,
