  ...
  return (
    <section className="mission-command-deck mission-command-deck-canvas" aria-label="Mission Command Deck">
      <div className="mission-command-deck-grid">
        ...
        <article className="mission-deck-card"><h4>Guardrail State</h4><p>OpenClaw authority: {openClawDelegation?.authorityLevel || 'unknown'} · approval gates: {openClawDelegation?.requiredOperatorApproval ? 'required' : 'not declared'} · verification: {verificationJudge?.judgment || 'pending'}.</p></article>
      </div>
    </section>
  );
}
