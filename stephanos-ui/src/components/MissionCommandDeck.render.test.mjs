...
test('MissionCommandDeck uses one canonical bounded root and scan-first grid sections', async () => {
  ...
  ['className="mission-command-deck mission-command-deck-canvas"', 'mission-command-deck-grid', 'mission-deck-grid-status-strip', 'mission-deck-grid-readiness-hero', 'mission-deck-grid-agent-assignment', 'mission-deck-grid-pr-repair', 'mission-deck-grid-operator-decision', 'mission-deck-grid-activity-feed', 'mission-deck-secondary-grid', 'mission-deck-secondary-card'].forEach((token) => {
...
test('Card grid and bounded card CSS guardrails are explicit', async () => {
  ...
  ['.mission-command-deck-grid', 'grid-template-columns:repeat(auto-fit,minmax(min(320px,100%),1fr));', '.mission-deck-grid-activity-feed { grid-column:1 / -1; }', '.mission-deck-secondary-grid { grid-column:1 / -1; display:grid; gap:12px; grid-template-columns:repeat(auto-fit,minmax(min(280px,100%),1fr));', '.mission-deck-card, .mission-deck-chip, .mission-deck-metric', 'min-inline-size:min(100%, 280px);', 'max-inline-size:100%;', 'box-sizing:border-box;', 'word-break:normal', '.mission-deck-actions { display:flex; gap:8px; flex-wrap:wrap;', '.mission-deck-preview-button', 'white-space:normal'].forEach((token) => {
...
test('MissionCommandDeck avoids unsafe vertical text/secondary squeeze patterns', async () => {
  ...
  ['@media (max-width: 980px) { .mission-command-deck-grid { grid-template-columns:minmax(0,1fr); }', '.mission-deck-grid-agent-assignment, .mission-deck-grid-pr-repair, .mission-deck-grid-operator-decision, .mission-deck-grid-activity-feed, .mission-deck-secondary-grid { grid-column:1 / -1;', '.mission-deck-secondary-grid { grid-template-columns:minmax(0,1fr); }'].forEach((token) => {
