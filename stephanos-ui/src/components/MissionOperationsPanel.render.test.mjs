...
test('Build Concierge panel renders selected candidate proof and approval truth', async () => {
  ...
  try {
    ...
    const markup = renderToStaticMarkup(React.createElement(BuildConciergeSurface, { concierge: {
      ...
      roadmap: { activePhase: { version: 'V4', title: 'Browser Proof Capture' }, phases: [{ version: 'V2', title: 'Operator Surfaces', status: 'implemented' }, { version: 'V3', title: 'Local Proof Runner', status: 'implemented_guarded' }, { version: 'V4', title: 'Browser Proof Capture', status: 'planned_guarded' }, { version: 'V5', title: 'Auto Pick Next Safe Work', status: 'planned_guarded' }, { version: 'V6', title: 'Operator Approval Surface', status: 'planned_guarded' }, { version: 'V7', title: 'Post-Merge Sync and Reproof', status: 'planned_guarded' }, { version: 'V8', title: 'Multi-Goal Queue', status: 'planned_guarded' }], successMarkers: ['GOAL_COMPLETE_BATTLE_BRIDGE_BUILD_CONCIERGE_ROADMAP', 'NO_CLICK_MONKEY_LOOP', 'INTENT_ENGINE_APPROVAL_ONLY'] },
    } }));
    for (const visibleValue of ['Build Concierge', '#1391', 'Battle Bridge Build Concierge V2 Operator Surfaces', 'ready', 'clean', 'npm test', 'HELD_PENDING_PROOF_PACKET_AND_EXACT_HEAD_APPROVAL', 'Run proof-packet for PR #1391.', 'V3', 'Local Proof Runner', 'GOAL_COMPLETE_BATTLE_BRIDGE_BUILD_CONCIERGE_ROADMAP', 'V4', 'Browser Proof Capture', 'V5', 'Auto Pick Next Safe Work', 'V6', 'Operator Approval Surface', 'V7', 'Post-Merge Sync and Reproof', 'V8', 'Multi-Goal Queue', 'planned_guarded']) {
      assert.equal(markup.includes(visibleValue), true, `missing rendered value: ${visibleValue}`);
    }
  } finally {
    await vite.close();
