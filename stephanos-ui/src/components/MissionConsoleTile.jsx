...
function MissionConsoleTile({
  ...
  const [builderWorkbenchInput, setBuilderWorkbenchInput] = useState(() => ({
    ...
    openClawSourcePackLastJudgedText: '',
    openClawSourcePackLastJudgedOutput: '',
    localAiReviewText: '',
            ...
            <CollapsiblePanel panelId="missionConsoleBuilderMeshPanel" title="Zero-Cost Builder Mesh V1" titleAs="h5" description={operatorReliefProjection.builderMeshProjection?.recommendedBuilder || 'routing pending'} isOpen={uiLayout.missionConsoleBuilderMeshPanel !== false} onToggle={() => dispatchPanelToggle('missionConsoleBuilderMeshPanel')}>
              ...
              <CollapsiblePanel panelId="missionConsoleBuilderWorkbenchPanel" title="Zero-Cost Builder Workbench V1" titleAs="h6" description={operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.workbenchStatus || 'ready'} isOpen={uiLayout.missionConsoleBuilderWorkbenchPanel !== false} onToggle={() => dispatchPanelToggle('missionConsoleBuilderWorkbenchPanel')}>
                <ul className="mission-console__status-list">
                  ...
                  <li><strong>OpenClaw Source Pack Judgment Stale:</strong> {operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.openClawSourcePackRunner?.sourcePackJudgmentStale || 'no'} · text {operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.openClawSourcePackRunner?.sourcePackLastJudgedTextLength ?? 0}/{operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.openClawSourcePackRunner?.sourcePackCurrentTextLength ?? 0} · output {operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.openClawSourcePackRunner?.sourcePackLastJudgedOutputLength ?? 0}/{operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.openClawSourcePackRunner?.sourcePackCurrentOutputLength ?? 0}</li>
                  <li><strong>OpenClaw Source Pack Projection:</strong> written {operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.openClawSourcePackRunner?.sourcePackProjectionWritten || 'no'} · source {operatorReliefProjection.builderMeshProjection?.builderWorkbenchProjection?.openClawSourcePackRunner?.sourcePackProjectionSource || 'source-pack-runner-idle'}</li>
                ...
                <div className="builder-workbench-output-card" data-testid="evidence-return-intake-section">
                  ...
                  <label className="mission-console__field-label builder-workbench-field-label">Paste Evidence Return Result<textarea className="builder-workbench-result-textarea builder-workbench-result-textarea--evidence-return" data-testid="builder-workbench-evidence-return-intake" value={builderWorkbenchInput.evidenceReturnIntakeText} onChange={(event) => setBuilderWorkbenchInput((prev) => ({ ...prev, activePacketType: 'evidence-return-intake', evidenceReturnIntakeText: event.target.value }))} placeholder="Paste returned proof here; deterministic intake will classify build-proof, verify-proof, browser-proof-checklist, pr-evidence, source-pack-output, or rejected/noise without unlocking mutation." /></label>
