...
import "./styles.css";
import {
  STEPHANOS_UI_BOOT_LOG,
  STEPHANOS_UI_BUILD_METADATA,
} from "./runtimeInfo";
...
async function logDevStartupHealthCheck() {
  ...
  const intendedMode =
    startupFingerprint.runtimeRole === "mission-control-dist-runtime"
      ? "4173 dist runtime"
      : "5173 Vite dev runtime";
  const intendedFinalUrl =
    startupFingerprint.runtimeRole === "mission-control-dist-runtime"
      ? canonicalUrls.runtimeIndexUrl
      : "http://127.0.0.1:5173/";
  const [port4173Running, port5173Running] = await Promise.all([
    fetch("http://127.0.0.1:4173/__stephanos/health", { method: "GET" })
      .then((response) => response.ok)
      .catch(() => false),
    fetch("http://127.0.0.1:5173/", { method: "GET" })
      .then((response) => response.ok)
      .catch(() => false),
  ]);

  console.info(
    "[Stephanos Runtime] Build metadata",
    STEPHANOS_UI_BUILD_METADATA,
  );
...
if (typeof installTopLevelCommandDeckReturnControls !== "function") {
  throw new Error(
    "installTopLevelCommandDeckReturnControls must be a function export from shared/runtime/commandDeckReturnControls.mjs",
  );
}
