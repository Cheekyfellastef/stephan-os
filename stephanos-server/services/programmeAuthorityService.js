...
function mergeGithubGoalEstate(workspaceGoalRecords, goalEstateRead, nowUtc) {
  ...
  for (const issue of goalEstateRead.issues) {
    ...
    observedRecords.push(Object.freeze({
      ...
      state: 'READY',
      prerequisites: [],
      priority: Number.isFinite(issue.priority) ? issue.priority : 0,
      criticalPathWeight: 0,
      goalClass: text(issue.goalClass, 'DURABLE_GOAL').toUpperCase(),
      automationDebtWeight: Number.isFinite(issue.automationDebtWeight) ? issue.automationDebtWeight : 0,
      recurrenceCount: Number.isFinite(issue.recurrenceCount) ? issue.recurrenceCount : 0,
      operatorToilWeight: Number.isFinite(issue.operatorToilWeight) ? issue.operatorToilWeight : 0,
      blastRadiusWeight: Number.isFinite(issue.blastRadiusWeight) ? issue.blastRadiusWeight : 0,
      effortWeight: Number.isFinite(issue.effortWeight) ? issue.effortWeight : 0,
      createdAt: safeNow(issue.createdAt) || null,
