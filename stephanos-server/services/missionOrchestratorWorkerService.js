...
function publicAction(action) {
  return {
    ...action,
    approvalToken: action.operation === 'merge-pr' ? action.approvalToken : '',
  };
}
...
export async function publishMissionWorkerAction(state, options = {}) {
  ...
  let payload = publicAction(action);
  ...
  const published = await atomicCreateJson(path, {
...
export async function collectAgentWorkerResult(result, options = {}) {
  ...
  if (!missionId || !actionId || !['codex', 'openclaw-readonly'].includes(adapter)) {
    throw new Error('Agent result identity is incomplete or unsupported.');
  }
  ...
  const expectedAdapter = current.state.dispatch.adapter;
  if (adapter !== expectedAdapter) throw new Error('Agent result adapter does not match the active dispatch.');
