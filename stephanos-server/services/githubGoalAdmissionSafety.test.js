...
test('ordinary goal and priority labels remain discovery-only and cannot manufacture scheduler READY authority', async () => { const result = await observe(canonicalGoal()); assert.equal(result.status, 'fetched'); assert.equal(result.admissionContractRequired, true); assert.equal(result.discoveredIssues.length, 1); assert.equal(result.discoveredIssues[0].issueNumber, 2314); assert.equal(result.discoveredIssues[0].admissionState, 'DISCOVERED_CANDIDATE'); assert.equal(result.discoveredIssues[0].schedulerEligible, false); assert.deepEqual(result.issues, []); });
test('one owner-authenticated closed-world comment admission makes a discovered goal scheduler eligible', async () => { const result = await observe(canonicalGoal(), [ownerAdmissionComment()]); assert.equal(result.issues.length, 1); const admitted = result.issues[0]; assert.equal(admitted.admissionState, 'ADMISSION_PROVEN'); assert.equal(admitted.admissionProofSource, 'OWNER_AUTHENTICATED_COMMENT'); assert.equal(admitted.schedulerEligible, true); assert.equal(admitted.admission.state, 'READY'); assert.equal(admitted.admission.route, 'OPENCLAW_LOCAL'); assert.equal(admitted.admission.sourceImplementationAllowed, true); assert.equal(admitted.admission.mergeAuthority, false); assert.equal(admitted.admission.deploymentAuthority, false); assert.equal(admitted.admission.runtimeMutationAuthority, false); assert.equal(admitted.admission.arbitraryShellAllowed, false); });
test('mutable issue body cannot grant admission even when timestamps and trusted creator metadata look unchanged', async () => { const result = await observe(canonicalGoal({ body: admissionBody(), created_at: '2026-09-21T00:00:00Z', updated_at: '2026-09-21T00:00:00Z', performed_via_github_app: { slug: 'chatgpt' } })); assert.equal(result.discoveredIssues.length, 1); assert.equal(result.discoveredIssues[0].schedulerEligible, false); assert.deepEqual(result.issues, []); });
test('non-owner comment cannot grant admission', async () => { const result = await observe(canonicalGoal(), [ownerAdmissionComment({ user: { login: 'collaborator' }, author_association: 'COLLABORATOR' })]); assert.equal(result.discoveredIssues.length, 1); assert.deepEqual(result.issues, []); });
test('owner comment cannot widen protected authority', async () => { const result = await observe(canonicalGoal(), [ownerAdmissionComment({ body: admissionBody({ mergeAuthority: true, deploymentAuthority: true, runtimeMutationAuthority: true, arbitraryShellAllowed: true }) })]); assert.equal(result.discoveredIssues.length, 1); assert.deepEqual(result.issues, []); });
test('multiple owner admission receipts fail closed as ambiguous', async () => { const result = await observe(canonicalGoal(), [ownerAdmissionComment(), ownerAdmissionComment()]); assert.equal(result.discoveredIssues.length, 1); assert.deepEqual(result.issues, []); });
test('comment read failure remains discovery-only and does not become authority', async () => { const result = await observe(canonicalGoal(), [], 503); assert.equal(result.status, 'fetched'); assert.equal(result.admissionReadFailureCount, 1); assert.equal(result.discoveredIssues.length, 1); assert.deepEqual(result.issues, []); });
...
test('production-style cache bounds repeated goal-estate observations until refresh expiry', async () => {
  ...
  const first = await fetchGithubGoalIssues(options); assert.equal(requestCount, 2); clockMs += 30_000; const cached = await fetchGithubGoalIssues(options); assert.equal(cached, first); assert.equal(requestCount, 2); clockMs += 31_000; const refreshed = await fetchGithubGoalIssues(options); assert.notEqual(refreshed, first); assert.equal(requestCount, 4); assert.equal(refreshed.issues.length, 1);
});
