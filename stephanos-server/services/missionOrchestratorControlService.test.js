...
const intent = {
  ...
  branch: 'orchestrator/control-service-test',
...
async function advanceToApproval(options, missionId) {
  await event(options, missionId, 'worktree-control-001', 'WORKTREE_READY', {
    worktreePath: intent.worktreePath,
    clean: true,
    receipt: proof('isolated worktree', 'worktree-receipt'),
  });
  await event(options, missionId, 'dispatch-control-001', 'AGENT_DISPATCHED', { agentId: 'codex' });
  await event(options, missionId, 'result-control-001', 'AGENT_RESULT_RECEIVED', {
    success: true,
    changedFiles: ['shared/agents/example.mjs'],
    receipt: proof('codex result', 'codex-receipt'),
  });
  await event(options, missionId, 'evidence-control-001', 'EVIDENCE_RECORDED', {
    receipts: [proof('focused test output', 'focused-test-receipt')],
  });
  await event(options, missionId, 'commit-control-001', 'GIT_OPERATION_COMPLETED', {
    operation: 'commit',
    commitSha: '1'.repeat(40),
    clean: true,
    receipt: proof('signed commit', 'commit-receipt'),
  });
  await event(options, missionId, 'push-control-001', 'GIT_OPERATION_COMPLETED', {
    operation: 'push',
    success: true,
    receipt: proof('signed push', 'push-receipt'),
  });
  await event(options, missionId, 'pr-control-001', 'PULL_REQUEST_OPENED', {
    prNumber: 1302,
    prUrl: 'https://github.com/Cheekyfellastef/stephan-os/pull/1302',
    headSha: '2'.repeat(40),
    mergeable: true,
    receipt: proof('open pull request', 'pr-receipt'),
  });
  await event(options, missionId, 'checks-control-001', 'PULL_REQUEST_CHECKS_UPDATED', {
    prNumber: 1302,
    headSha: '2'.repeat(40),
    prState: 'open',
    mergeable: true,
    checks: [{ name: 'Build', status: 'success', required: true }],
    receipt: proof('pull request checks', 'checks-receipt'),
  });
}

test('bounded control creates, lists, and reads missions without accepting arbitrary state', async () => {
  ...
  const listed = await listBoundedMissions(options);
  assert.equal(listed.length, 1);
  const read = await readBoundedMission(intent.missionId, options);
...
test('exact approval command advances once and the same command is retry-safe', async () => {
  ...
  assert.equal(ready.state.currentPhase, 'AWAITING_OPERATOR_APPROVAL');
  const command = {
    missionId: intent.missionId,
    commandId: 'operator-command-001',
    approvalToken: ready.state.approval.requiredToken,
  };
...
test('incorrect approval is rejected before an event is appended', async () => {
  ...
  await createBoundedMission(intent, options);
  await advanceToApproval(options, intent.missionId);
  await assert.rejects(() => approveBoundedMission({
    missionId: intent.missionId,
    commandId: 'operator-command-002',
    approvalToken: 'APPROVE_OPENCLAW_SQUASH_MERGE:1302:stale',
  }, options), /does not match/);
  const current = await readBoundedMission(intent.missionId, options);
